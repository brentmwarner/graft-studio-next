# Plan 006: Make Graft the Authoritative Agent Harness
...
An explicit request to start two Graft threads produced six threads. The
...
`graft_create_thread`, tried several invalid provider/model combinations, and
...
1. **Model awareness** — every provider is told, through its strongest native
   instruction channel, that Graft is the hosting harness and that Graft
   resources are controlled through `graft_*` tools.
2. **Capability truth** — agents can ask Graft which providers, models,
...
```text
user asks for exactly two Graft threads
  -> parent reads graft_context / graft_capabilities
  -> parent calls graft_create_threads once with exactly two entries
  ...
  -> parent calls graft_wait_for_threads until both runs are terminal
...
- Graft is the harness; provider-native subagents are implementation details
  inside a provider turn and never substitute for user-requested Graft
  threads.
- All model-facing host text comes from one versioned
  `GraftHarnessPolicy`. Provider adapters only choose the delivery mechanism.
- Every state-changing gateway call is bound to a live Graft provider session
...
- Unsupported provider/model/options fail before `thread.create`, git branch,
  worktree, or provider side effects. Graft never interprets `Low` as part of
...
- `apps/server/src/agentGateway/Layers/AgentGateway.ts:73-81` owns the only full
  “Graft is hosting this session / use graft\_\*” policy, but
...
- `apps/server/src/provider/Layers/ClaudeAdapter.ts:880-885` tells Claude it is
  running inside Graft, but does not define Graft-thread versus native
  subagent precedence.
- `apps/server/src/codexAppServerManager.ts:475-486,620-648` sends Codex
  ...
  include the Graft harness contract.
- Cursor, Grok, and Droid receive MCP transport configuration and a
  Graft `clientInfo`, but no shared model-facing host policy. See
...
- `apps/server/src/provider/runtimeLayer.ts:54-70` explicitly excludes OpenCode
  ...
  and a clear “Graft control unavailable” capability instead of pretending
  parity.
...
- A background native agent that retains the same MCP connection can therefore
  mutate Graft after the visible parent turn has completed.
...
- predecessor naming — Graft identity must remain consistent;
...
### `graft_context` (read-only, idempotent)

```ts
type GraftContextResult = {
  harness: { name: "Graft"; policyVersion: string };
  caller: {
    threadId: ThreadId;
    turnId: TurnId | null;
...
### `graft_capabilities` (read-only, idempotent)
...
### `graft_create_threads` (destructive, idempotent with request id)
...
Keep `graft_create_thread` as a backwards-compatible single-entry wrapper
...
### `graft_wait_for_threads` (read-only, idempotent)

```ts
type GraftWaitForThreadsInput = {
...
- Rebranding or renaming `graft_*` tools.
- Disabling provider-native subagents globally. They remain available for
  ...
  explicit Graft-resource requests.
...
- UI redesign. Existing Graft MCP logo/labels may consume tool annotations,
...
- Branch from the current feature branch with `codex/graft-harness-control-plane`.
...
### Step 2: Centralize and deliver the Graft harness policy
...
1. “You are running inside Graft. Graft is the host/harness for this
   session.”
2. Use `graft_*` for Graft threads, projects, automations, and coordination.
3. Provider-native `spawn_subagent`/Task/collaboration tools do not create
   Graft threads and must not substitute for an explicit request for Graft
   threads.
4. Use one `graft_create_threads` call for plural requests; the array count is
   exact.
5. Resolve canonical models/options through `graft_capabilities`; do not guess
   slugs or silently change provider/model.
6. Wait with `graft_wait_for_threads`, then synthesize all results.
...
- **Claude**: append it to the existing Claude system-prompt append. Add MCP
  ...
  list.
- **Cursor/Grok/Droid ACP paths**: include consistent Graft clientInfo
  ...
  user-visible Graft message projection.
- **OpenCode/Kilo**: deliver truthful host identity, but advertise Graft MCP
...
- **Pi**: deliver host identity through its supported prompt/extension path and
  explicitly advertise that Graft MCP control is unavailable. Do not simulate
  successful Graft actions.

Keep MCP initialize `instructions` generated from the same policy. Add tool
annotations and make every description begin with its user-facing Graft
...
- caller thread id;
- a Graft-generated provider-session key (not an upstream provider id);
- provider kind;
...
- return the same catalog from `graft_capabilities` that validation uses;
...
Implement `graft_wait_for_threads` as a read-only long poll over projections:
...
This cap protects Graft projections from provider fan-out, but it does not
disable native subagent execution. Keep native-child budgeting separate from
the ordinary top-level `graft_create_threads` operation.
...
- the active parent path uses one two-entry Graft batch;
...
- [x] Every provider receives the same versioned Graft host identity through
      its strongest available instruction channel.
- [x] Every provider truthfully reports whether it can call Graft MCP; no
      provider simulates unsupported host actions.
- [x] MCP initialize identity, tool descriptions, and native host instructions
      all say Graft, never a predecessor or provider-branded harness.
- [x] Every destructive call requires a live scoped provider session and active
      caller turn.
- [x] A detached background agent cannot create or mutate Graft threads after
...
- `GraftHarnessPolicy` is the single source of truth. Any new provider must add
