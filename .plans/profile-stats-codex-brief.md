...
You are GPT-5.5 (xhigh) working in the Graft monorepo at `/Users/emanueledipietro/Developer/graft`.
...
- BRANCH dev DB (what the dev instance uses): `./.graft/electron-dev/dev/state.sqlite`  (~40 turns)
- REAL app DB: `~/.graft/userdata/state.sqlite`  (~437 turns, 192 MB) — the important one
...
2. `npx turbo run typecheck --filter=@graft/contracts --filter=@graft/cli` must pass. DO NOT run the web
   (`@graft/web`) typecheck — its UI is being rewritten in parallel and will be red until Opus finishes.
