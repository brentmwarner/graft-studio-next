...
You are GPT-5.5 (xhigh) in the Graft monorepo at `/Users/emanueledipietro/Developer/graft`.
...
- `config.homeDir` = `OS.homedir()` ALWAYS (`apps/server/src/main.ts:172,217`) — it is NOT affected by
  `--home-dir`.
- `config.baseDir` = `--home-dir` / `GRAFT_HOME` (dev: `./.graft/electron-dev`; prod: `~/.graft` or
  `~/Documents/Graft`). `config.dbPath` = `baseDir/{dev|userdata}/state.sqlite`.
...
- The dev runner sets `GRAFT_HOME/GRAFT_HOME/GRAFT_HOME=baseDir` but does NOT set `CODEX_HOME` or
...
5. **Run the checks** to back every claim:
   - `sqlite3` on `./.graft/electron-dev/dev/state.sqlite` (dev) and `~/.graft/userdata/state.sqlite`
     (prod) where relevant.
