...
- Mobile lifecycle commands now reject hidden Studio threads before changing their title or archive state or deleting them. ([#77](https://github.com/brentmwarner/graft-studio-next/pull/77))
- Phone thread titles refresh during long responses and keep manual renames; updated iOS source also replaces the running-thread spinner with an activity arc that respects Reduce Motion. These phone interface changes need separate mobile app releases. ([#80](https://github.com/brentmwarner/graft-studio-next/pull/80))
