...
function createAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "graft-artifact-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "Graft-1.2.3-x64.AppImage"), "app-image-bytes");
  writeFileSync(join(root, "latest-linux.yml"), "version: 1.2.3\n");
  return root;
}

function createWindowsAssets(): string {
  const root = mkdtempSync(join(tmpdir(), "graft-windows-provenance-test-"));
  temporaryRoots.push(root);
  writeFileSync(join(root, "Graft-1.2.3-x64.exe"), "unsigned-windows-bytes");
...
describe("release artifact provenance", () => {
  it("hashes the exact collected Linux assets into a deterministic manifest", async () => {
    ...
    expect(result.manifest.artifacts.map((artifact) => artifact.fileName)).toEqual([
      "Graft-1.2.3-x64.AppImage",
      "latest-linux.yml",
    ]);
    expect(
      result.manifest.artifacts.find((artifact) => artifact.fileName === "Graft-1.2.3-x64.AppImage")
        ?.sha256,
