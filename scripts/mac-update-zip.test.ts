...
`,
  ...
  it("builds Electron framework symlink paths for the top-level app bundle", () => {
    assert.deepStrictEqual(buildMacUpdateZipSymlinkEntries("Graft.app"), [
      "Graft.app/Contents/Frameworks/Electron Framework.framework/Electron Framework",
      "Graft.app/Contents/Frameworks/Electron Framework.framework/Helpers",
      "Graft.app/Contents/Frameworks/Electron Framework.framework/Libraries",
      "Graft.app/Contents/Frameworks/Electron Framework.framework/Resources",
      "Graft.app/Contents/Frameworks/Electron Framework.framework/Versions/Current",
    ]);
  });
  ...
  it("resolves exactly one top-level .app from update zip entries", () => {
    assert.equal(
      resolveSingleTopLevelMacAppBundle([
        "__MACOSX/Graft.app/Contents/Info.plist",
        "Graft.app/Contents/Info.plist",
  ...
  it("resolves exactly one macOS update zip artifact", () => {
    assert.equal(
      resolveSingleMacUpdateZipFileName([
        "Graft-0.1.5-arm64.dmg",
        "Graft-0.1.5-arm64.zip",
        "latest-mac.yml",
      ]),
      "Graft-0.1.5-arm64.zip",
    );
    ...
    assert.throws(
      () => resolveSingleMacUpdateZipFileName(["Graft-0.1.5-arm64.zip", "Graft-0.1.5-x64.zip"]),
  ...
  it("requires at least one macOS update manifest", () => {
    ...
    assert.throws(
      () => resolveMacUpdateManifestFileNames(["Graft-0.1.5-arm64.dmg"]),
...
files:
  - url: Graft-0.1.4-arm64.zip
    sha512: oldzip
    size: 100
  - url: Graft-0.1.4-arm64.dmg
    sha512: olddmg
    size: 200
path: 'Graft-0.1.4-arm64.zip'
sha512: oldzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;

    const updated = updateMacUpdateManifestZipEntry(manifest, "Graft-0.1.4-arm64.zip", {
      sha512: "newzip",
      size: 12345,
    });
...
releaseDate: '2026-06-07T12:00:00.000Z'
`,
          "Graft-0.1.4-arm64.zip",
          {
            sha512: "newzip",
            size: 12345,
          },
        ),
      /Could not update Graft-0.1.4-arm64.zip entry/,
    );
  });
...
files:
  - url: Graft-0.1.5-arm64.zip
    sha512: newzip
    size: 12345
path: Graft-0.1.5-arm64.zip
sha512: newzip
releaseDate: '2026-06-07T12:00:00.000Z'
`;
    ...
    assert.deepStrictEqual(
      validateMacUpdateManifestZipMetadata(manifest, "Graft-0.1.5-arm64.zip", metadata),
      {
    ...
    assert.deepStrictEqual(
      assertMacUpdateManifestZipMetadata(manifest, "Graft-0.1.5-arm64.zip", metadata),
      {
        manifestHasZipPath: true,
        manifestHasZipSha: true,
        manifestHasZipSize: true,
