...
export interface OccupancyStoredReceipt {
  sessionId: string;
  commandId: string;
  requestHash: string;
...
export type OccupancyCommandReservation =
  | { kind: "new"; receipt: OccupancyStoredReceipt }
  | { kind: "replay"; receipt: OccupancyStoredReceipt }
  | { kind: "conflict"; receipt: OccupancyStoredReceipt };

export interface OccupancyStoredEvent {
  cursor: number;
  occurredAt: number;
  event: GraftDesktopJsonValue;
}

export type OccupancyReplayResult =
  | {
      kind: "events";
      cursor: number;
      replayFloor: number;
      events: OccupancyStoredEvent[];
    }
  | {
      kind: "cursor_expired";
      cursor: number;
...
export interface OccupancyStoreOptions {
  now?: () => number;
  randomSecret?: () => string;
}
...
function sessionFromRow(row: SessionRow): GraftDesktopSessionRecord {
  return GraftDesktopSessionRecordSchema.parse({
    ...
    grants: GraftDesktopCapabilityListSchema.parse(JSON.parse(row.grantsJson)),
...
function receiptFromRow(row: ReceiptRow): OccupancyStoredReceipt {
  const receipt: OccupancyStoredReceipt = {
    sessionId: row.sessionId,
  ...
  if (row.resultJson !== null) {
    receipt.result = GraftDesktopJsonValueSchema.parse(JSON.parse(row.resultJson));
  }
  if (row.errorJson !== null) {
    receipt.error = GraftDesktopErrorSchema.parse(JSON.parse(row.errorJson));
  }
  return receipt;
}
...
export class OccupancyStore {
  ...
  constructor(dbPath: string, options: OccupancyStoreOptions = {}) {
  ...
  private migrate(): void {
    ...
    if (!current) {
      this.db.prepare("INSERT INTO desktop_host_schema_version (version) VALUES (0)").run();
    }
  ...
  getOrCreateIdentity(environmentLabel: string): OccupancyIdentity {
    const existing = this.db
      .prepare(
        "SELECT environmentId, environmentLabel, createdAt FROM desktop_host_identity WHERE singleton = 1",
      )
      .get() as OccupancyIdentity | undefined;
    if (existing) return existing;

    const identity: OccupancyIdentity = {
    ...
    this.db
      ...
      .run(identity.environmentId, identity.environmentLabel, identity.createdAt);
    return identity;
  }

  getIdentity(): OccupancyIdentity | null {
    return (
      (this.db
        .prepare(
          "SELECT environmentId, environmentLabel, createdAt FROM desktop_host_identity WHERE singleton = 1",
        )
        .get() as OccupancyIdentity | undefined) ?? null
    );
  }
  ...
  consumeEnrollmentToken(input: {
    ...
    sessionTtlMs?: number;
  }): OccupancyEnrollment | null {
    const identity = this.getIdentity();
    if (!identity) throw new Error("Desktop host identity has not been initialized");
    ...
    try {
      const token = this.db
        ...
        .get(tokenDigest) as { expiresAt: number; consumedAt: number | null } | undefined;
  ...
  authenticateBearer(bearer: string): GraftDesktopSessionRecord | null {
    ...
    const row = this.db
      ...
      .get(digestSecret(bearer)) as SessionRow | undefined;
    if (!row || row.revokedAt !== null || row.expiresAt <= now) return null;
    this.db
      .prepare("UPDATE desktop_host_sessions SET lastSeenAt = ? WHERE sessionId = ?")
  ...
  reserveCommand(input: {
    ...
    try {
      const existing = this.db
        ...
        .get(input.sessionId, input.commandId) as ReceiptRow | undefined;
      if (existing) {
        this.db.exec("COMMIT");
        return {
          kind: existing.requestHash === input.requestHash ? "replay" : "conflict",
  ...
  completeCommand(input: {
    ...
    const updated = this.db
      ...
      .run(JSON.stringify(result), completedAt, input.sessionId, input.commandId);
  ...
  failCommand(input: {
    ...
    const updated = this.db
      ...
      .run(JSON.stringify(error), completedAt, input.sessionId, input.commandId);
    if (updated.changes !== 1) {
      throw new Error("Desktop command receipt was not in the accepted state");
    }
    return this.requireReceipt(input.sessionId, input.commandId);
  ...
  private requireReceipt(sessionId: string, commandId: string): OccupancyStoredReceipt {
    const row = this.db
      .prepare(
        `SELECT sessionId, commandId, requestHash, status, resultJson,
                errorJson, acceptedAt, completedAt
      ...
      .get(sessionId, commandId) as ReceiptRow | undefined;
    if (!row) throw new Error("Desktop command receipt is missing");
    return receiptFromRow(row);
  }

  appendEvent(event: GraftDesktopJsonValue): OccupancyStoredEvent {
    ...
    try {
      ...
      this.db
        .prepare("INSERT INTO desktop_host_events (cursor, occurredAt, eventJson) VALUES (?, ?, ?)")
        .run(cursor, occurredAt, JSON.stringify(parsed));
      this.db.prepare("UPDATE desktop_host_state SET cursor = ? WHERE singleton = 1").run(cursor);
      this.db.exec("COMMIT");
  ...
  replayEvents(afterCursor: number, limit = 1_000): OccupancyReplayResult {
    const state = this.getReplayState();
    ...
    const rows = this.db
      ...
      .all(afterCursor, limit) as Array<{ cursor: number; occurredAt: number; eventJson: string }>;
    return {
      kind: "events",
      ...state,
      events: rows.map((row) => ({
        ...
        event: GraftDesktopJsonValueSchema.parse(JSON.parse(row.eventJson)),
      })),
    };
  }
  ...
  getReplayState(): { cursor: number; replayFloor: number } {
    return this.db
      .prepare("SELECT cursor, replayFloor FROM desktop_host_state WHERE singleton = 1")
