...
function makeTemporaryRoot(): string {
  const root = FS.mkdtempSync(Path.join(OS.tmpdir(), "graft-login-shell-"));
...
describe("createCachedLoginShellEnvironmentReader", () => {
  ...
  it("runs uncached when the home directory cannot be resolved", () => {
    ...
    // call-count assertion below) instead of touching the developer's real Graft home.
    const env = { GRAFT_HOME: Path.dirname(Path.dirname(fixture.cachePath)) };
...
describe("loginShellEnvironmentCachePath", () => {
  it("anchors the cache in the Graft home both processes resolve", () => {
    expect(loginShellEnvironmentCachePath({ env: { GRAFT_HOME: "/tmp/graft-home" } })).toBe(
      Path.join("/tmp/graft-home", "cache", "login-shell-environment.json"),
    );
