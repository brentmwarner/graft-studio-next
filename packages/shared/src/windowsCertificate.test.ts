  ...
  it("matches expected fields independent of order and extra certificate fields", () => {
    expect(
      matchesDistinguishedName(
        ...
        "C=US, O=Acme Tools, CN=Graft, SERIALNUMBER=1234",
      ),
    ).toBe(true);
  });

  it("rejects incomplete pins and mismatched signer fields", () => {
    expect(matchesDistinguishedName("CN=Graft", "CN=Graft, O=Acme Tools")).toBe(false);
    expect(matchesDistinguishedName("CN=Graft, O=Acme Tools", "CN=Graft, O=Other Tools")).toBe(
      false,
    );
  });
});
