...
/**
 * Who owns the boot. Graft only auto-shuts down devices it booted itself;
 * anything the user started (pane picker, Simulator.app) outlives the session.
 */
export const DeviceBootSource = Schema.Literals(["graft", "synara", "user"]);
...
export const ThreadDeviceState = Schema.Struct({
  ...
  /** Devices the pane may show: booted devices plus anything Graft is booting. */
...
/** Devices Graft itself booted are capped globally; viewing already-booted devices is not. */
export const DEVICE_GRAFT_BOOT_LIMIT = 3;
...
/**
 * Boot is refusable rather than fatal: past DEVICE_GRAFT_BOOT_LIMIT the caller
...
export const DeviceBootResult = Schema.Union([
  ...
  Schema.Struct({
    ...
    graftBooted: Schema.Array(DeviceDescriptor).check(Schema.isMaxLength(64)),
  }),
]);
...
export const DeviceInstallAppInput = Schema.Struct({
  udid: DeviceUdid,
  /** Absolute path to a built `.app` bundle; Graft never runs the build itself. */
 ...
 * the bitstream. Runtime encode/decode lives in `@graft/shared/deviceFrame`;
