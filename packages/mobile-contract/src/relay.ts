...
/** Desktop-to-backend status; credentials never cross this boundary. */
export const GraftDesktopRelayStatusSchema = z.object({
  state: z.enum(["disabled", "connecting", "connected", "error"]),
  httpBaseUrl: z.string().url().nullable(),
  wsBaseUrl: z.string().url().nullable(),
  lastError: z.string().max(1024).nullable(),
});
export type GraftDesktopRelayStatus = z.infer<typeof GraftDesktopRelayStatusSchema>;
