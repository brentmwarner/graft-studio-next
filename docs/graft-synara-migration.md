...
- `packages/mobile-contract` — the versioned, Graft-owned wire contract shared
  by Android and the future Synara server adapter; its golden fixtures also
...
1. Develop on `codex/graft-synara-migration`; keep both legacy `main` branches
   protected.
2. Implement the Graft mobile HTTP/WebSocket adapter over Synara orchestration
   before changing either mobile client's behavior.
