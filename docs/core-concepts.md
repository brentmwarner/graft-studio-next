...
Graft becomes much easier to use once its ownership model is clear: **each task owns one body of
...
| Concept          | Meaning                                                             |
| ---------------- | ------------------------------------------------------------------- |
| Workspace        | The complete Graft application and the projects available in it     |
...
A project is the folder Graft works with.
...
[thread goal](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/features/thread-goals). A goal can continue after a
clean turn, but queued user work, approvals, questions, interruptions, failures, and pause rules
remain in control.

Use a [thread fork](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/workflows/forks) when a new task should inherit
the conversation or split from one exact turn. Use a
[handoff](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/workflows/handoffs) when another provider should continue
...
Read the [Git worktrees guide](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/workflows/worktrees) before starting
several tasks in the same repository.

## Providers, models, and sessions

A provider is the coding-agent runtime Graft operates, such as Claude Code, Codex, OpenCode, Cursor,
...
Graft supplies the shared workspace around it:
...
Each task owns a provider session. Available models and controls can differ because Graft discovers
capabilities from the installed runtime and account.

## Handoffs

A [provider handoff](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/workflows/handoffs) lets another supported
provider continue the same task and working environment using the context Graft passes to it.
...
Graft treats Git as the durable review and recovery layer.
...
Graft's checkpoint and revert controls can help recover task work, but committed Git history remains
...
Read the [parallel agents guide](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/workflows/parallel-agents) before
...
Check the [keyboard reference](https://github.com/brentmwarner/graft-studio-next/tree/main/docs/reference/keyboard-shortcuts) for the
