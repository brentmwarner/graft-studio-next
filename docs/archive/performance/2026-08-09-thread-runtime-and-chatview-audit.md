...
- Electron owns one main process, one primary browser renderer, Chromium GPU/utility
  processes, and one Graft backend child. These are application infrastructure,
...
belong to that provider. Graft's additional cost is its Electron/backend process
...
The page exposes `window.__graftPerf.snapshot()`, `resetMetrics()`,
...
```sh
env -u GRAFT_AUTH_TOKEN GRAFT_PORT_OFFSET=4117 GRAFT_NO_BROWSER=1 \
  bun run dev -- --home-dir ./.graft-perf-audit --port 59231
```
...
| Scenario                                             | Graft CPU / memory                                                                            | Provider CPU / memory                                                              | Processes                                                               | Renderer/frame evidence                                                    | Observation                                                                                                                                                                                                                                                                                           |
| ---------------------------------------------------- | --------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | -------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
| Packaged backend, five-second sample                 | 269 MB physical, 387 MB peak; 4,110/4,211 main-thread samples in `kevent`                     | Not included                                                                       | One Graft backend                                                       | n/a                                                                        | The observed 7.9% was transient, not sustained backend CPU.                                                                                                                                                                                                                                           |
...
| Equivalent task through Graft                        | About 93 MB dev backend at the sampled instant                                                | Approximately 156 MB for the provider pair, approximately 0% CPU after completion  | Graft backend/browser plus provider tree                                | Response completed in 23 seconds                                           | The provider RSS was essentially the same as direct; Graft adds its own backend/browser processes and event pipeline. Model differed (GPT-5.5 through Graft versus GPT-5.6 direct), so wall time is not a valid latency comparison.                                                                   |
...
### 1. Durable journal safety polling (medium/high idle impact, Graft-owned)
...
### 2. Unrelated streaming updates scanned retained thread entries (medium under concurrency, Graft renderer-owned)
...
### 3. Infinite ChatView working shimmer (medium steady renderer impact, Graft-owned; retained by design)
...
### 4. Claude lifecycle lock map retained every historical thread (low per thread, unbounded, Graft-owned)
...
### 5. Per-event durable journal cost (medium/high while streaming, Graft-owned; measured but not changed)
...
a transaction. This maps provider chunk frequency directly to Graft SQLite work.
...
- Shutdown PID audit found zero remaining descendants from the isolated Graft tree.
- `bun fmt`: passed.
