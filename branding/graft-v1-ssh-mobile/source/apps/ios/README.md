...
apps/ios/
  ...
  GraftTests/
    ...
    Fixtures/
      mobile-v1/                       # Copied from packages/shared/protocol-fixtures/
        *.json
  README.md
```
...
`GraftTests/Fixtures/mobile-v1/` contains a copy of `packages/shared/protocol-fixtures/mobile-v1/valid/`.

Check fixture parity from the repository root:

```bash
pnpm check:ios-fixtures
```

After intentionally changing shared fixtures, refresh the byte-for-byte iOS
copy with `pnpm sync:ios-fixtures`. CI runs the check and reports missing,
