...
import { SshRemoteConnectionManager } from "./sshRemoteConnectionManager.js";
...
class MemorySecretStore implements RemoteSessionSecretStore {
  ...
  set(accountKey: string, secret: string): void {
    this.values.set(accountKey, secret);
  }
...
describe("SshRemoteConnectionManager", () => {
  it("resolves, bootstraps, tunnels, enrolls, and stores the bearer only in secure storage", async () => {
    ...
    const commandRunner = vi.fn<SshCommandRunner>(
    ...
    const databasePath = join(tmpdir(), `graft-ssh-manager-${randomUUID()}.db`);
    paths.push(databasePath);
    const machineStore = new SshMachineStore(databasePath);
    ...
    expect(connection.routes.httpBaseUrl).toBe(
  ...
  it("revokes a newly enrolled session when secure storage rejects its bearer", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn<typeof fetch>(async (input, init) => {
        ...
        if (
          url.endsWith(GRAFT_DESKTOP_ENDPOINTS.session) &&
          init?.method === "DELETE"
        ) {
    ...
    paths.push(databasePath);
    const machineStore = new SshMachineStore(databasePath);
    const secretStore: RemoteSessionSecretStore = {
      get: () => null,
      set: () => {
  ...
  it("reuses an in-flight or active connection instead of tearing the tunnel down", async () => {
    const environmentId = "host-fedora-reuse-connection";
    const bearer = "desktop-bearer-value-0000000000000000003";
    let enrollments = 0;
    let releaseHealth: (() => void) | undefined;
    const holdHealth = new Promise<void>((resolve) => {
        ...
        if (url.endsWith(GRAFT_DESKTOP_ENDPOINTS.enroll)) {
          ...
          return new Response(
            JSON.stringify({
              protocolVersion: GRAFT_DESKTOP_PROTOCOL_VERSION,
              session: {
                sessionId: "session-reuse-connection",
                environmentId,
                profile: "desktop_occupancy",
                clientId: "desktop-client-01",
                clientLabel: "Brent's Mac",
    ...
    const first = manager.connect(machine.id);
    const second = manager.connect(machine.id);
    releaseHealth?.();
    const [firstConnection, secondConnection] = await Promise.all([
      first,
      second,
    ]);
    const thirdConnection = await manager.connect(machine.id);

    expect(secondConnection).toBe(firstConnection);
    expect(thirdConnection).toBe(firstConnection);
    expect(enrollments).toBe(1);
    expect(manager.activeConnection(machine.id)).toBe(firstConnection);
    await manager.disconnect(machine.id);
    machineStore.close();
  });

  it("does not reuse a connection whose close is still in flight", async () => {
    const environmentId = "host-fedora-closing-connection";
    let enrollments = 0;
    vi.stubGlobal(
      "fetch",
      vi.fn<typeof fetch>(async (input, init) => {
        ...
        if (url.endsWith(GRAFT_DESKTOP_ENDPOINTS.enroll)) {
          ...
          return new Response(
            JSON.stringify({
              protocolVersion: GRAFT_DESKTOP_PROTOCOL_VERSION,
              session: {
                sessionId: `session-closing-connection-${enrollments}`,
                environmentId,
                profile: "desktop_occupancy",
                clientId: "desktop-client-01",
                clientLabel: "Brent's Mac",
              ...
              bearer: `desktop-bearer-closing-connection-${enrollments}`,
            }),
            { status: 200 },
          );
        }
    ...
    const manager = new SshRemoteConnectionManager({
      ...
      createTunnel: (options) => {
        ...
        const originalClose = tunnel.close.bind(tunnel);
        tunnel.close = async () => {
          await holdClose;
          await originalClose();
        };
    ...
    const firstConnection = await manager.connect(machine.id);
    const closing = firstConnection.close();
    const reconnecting = manager.connect(machine.id);
    let reconnectedEarly = false;
    void reconnecting.then(() => {
      reconnectedEarly = true;
    });
    await Promise.resolve();
    expect(reconnectedEarly).toBe(false);
    expect(enrollments).toBe(1);

    releaseClose?.();
    const nextConnection = await reconnecting;
    await closing;
    expect(nextConnection).not.toBe(firstConnection);
    expect(enrollments).toBe(2);
    expect(manager.activeConnection(machine.id)).toBe(nextConnection);
    await manager.disconnect(machine.id);
    machineStore.close();
  });
});
