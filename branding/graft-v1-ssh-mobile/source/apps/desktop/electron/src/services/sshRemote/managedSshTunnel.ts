...
import { classifySshFailure } from "./sshTarget.js";
...
async function probeHealth(
  localPort: number,
): Promise<GraftDesktopHealth | null> {
  try {
    const response = await fetch(
      `http://127.0.0.1:${localPort}${GRAFT_DESKTOP_ENDPOINTS.health}`,
...
export class ManagedSshTunnel {
  ...
  private readonly spawnProcess: NonNullable<
    ManagedSshTunnelOptions["spawnProcess"]
  >;
  private readonly healthProbe: NonNullable<
    ManagedSshTunnelOptions["healthProbe"]
  >;
  private readonly reservePort: NonNullable<
    ManagedSshTunnelOptions["reservePort"]
  >;
  ...
  get localPort(): number {
    if (this.localPortValue === null)
      throw new Error("SSH tunnel is not started");
  ...
  private async openProcess(): Promise<void> {
    ...
    try {
      ...
      if (this.child !== child)
        throw new Error("SSH tunnel process changed while opening");
