...
const summary: GraftDiffSummary = { id: "diff", threadId: "t", runId: "r1", updatedAt: 1, files: [{ path: "first.ts", status: "modified" }, { path: "second.ts", status: "added" }] };
const detailed = { ...summary, files: summary.files.map((file) => ({ ...file, detailStatus: "ready" as const, hunks: [] })) };
let renderer: ReactTestRenderer;
let state: ReturnType<typeof useDiffFiles>;
function Harness({ diff = summary, visible = true, load }: { diff?: GraftDiffSummary; visible?: boolean; load: (thread: string, path: string) => Promise<GraftDiffSummary | undefined> }) {
  state = useDiffFiles(diff, visible, load);
  return null;
}
beforeEach(() => vi.stubGlobal("IS_REACT_ACT_ENVIRONMENT", true));
afterEach(async () => { if (renderer) await act(() => renderer.unmount()); vi.unstubAllGlobals(); });
...
describe("expanded diff file loading", () => {
  it("loads only the first file, then caches other files on expansion", async () => {
    ...
    await act(async () => { renderer = create(createElement(Harness, { load })); });
  ...
  it("rejects a reply for a newer checkpoint and allows retry", async () => {
    const load = vi.fn().mockResolvedValueOnce({ ...detailed, updatedAt: 2 }).mockResolvedValueOnce(detailed);
    await act(async () => { renderer = create(createElement(Harness, { load })); });
  ...
  it("ignores a response after the sheet closes", async () => {
    ...
    const load = vi.fn(() => new Promise<GraftDiffSummary>((done) => { resolve = done; }));
    await act(() => { renderer = create(createElement(Harness, { load })); });
