...
const scroll = vi.fn();
const answer: TranscriptItem = { id: "answer", kind: "assistant", text: "Hello", reasoning: "", streaming: true };
function Harness({ items, canStream = true }: { items: readonly TranscriptItem[]; canStream?: boolean }) {
...
function scrollEvent(offset: number, velocity = 0): NativeSyntheticEvent<NativeScrollEvent> {
  return { nativeEvent: { contentOffset: { y: offset }, contentSize: { height: 1000 }, layoutMeasurement: { height: 400 }, velocity: { y: velocity } } } as NativeSyntheticEvent<NativeScrollEvent>;
}
...
async function layout(height = 1000) {
  await act(() => { follow.handleContentSizeChange(400, height); vi.runAllTimers(); });
}
...
beforeEach(async () => {
  ...
  await act(() => { renderer = create(createElement(Harness, { items: [answer] })); });
  follow.listRef.current = { scrollToOffset: scroll } as unknown as NonNullable<Follow["listRef"]["current"]>;
  await layout();
  scroll.mockClear();
});
afterEach(async () => {
...
describe("transcript follow", () => {
  ...
  it("preserves reading position through reconnect until jump to latest", async () => {
    ...
    expect(scroll).not.toHaveBeenCalled();
    await act(() => { follow.jumpToLatest(); vi.runAllTimers(); });
    expect(scroll).toHaveBeenCalledExactlyOnceWith({ offset: 1200, animated: true });
  });
  it("does not follow tool-only growth", async () => {
    const tool: TranscriptItem = { id: "tool", kind: "tool", toolId: "tool", name: "Bash", detail: "Working", running: true };
    await update([{ ...answer, streaming: false }, tool]);
    await layout(1200);
    await update([{ ...answer, streaming: false }, { ...tool, detail: "More tool output" }]);
    await layout(1300);
    expect(scroll).not.toHaveBeenCalled();
  });
  it("cancels a queued follow when the user starts scrolling", async () => {
    await update([{ ...answer, text: "More output" }]);
    await act(() => { follow.handleScrollBeginDrag(); vi.runAllTimers(); });
    expect(scroll).not.toHaveBeenCalled();
  });
});
