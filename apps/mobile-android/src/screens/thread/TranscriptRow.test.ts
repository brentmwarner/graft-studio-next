...
vi.mock("react-native", () => ({
  ...
  useColorScheme: () => "light", Linking: { openURL: vi.fn() },
}));
vi.mock("@expo/vector-icons", () => ({ Ionicons: "Icon" }));
vi.mock("@expo/ui/jetpack-compose", () => ({
  Host: "ComposeHost", Text: "ComposeText",
}));
...
it("renders running reasoning and tools without another live indicator", async () => {
  const items: TranscriptItem[] = [
    ...
    { id: "group", kind: "toolGroup", tools: [{ id: "t2", kind: "tool", toolId: "t2", name: "Bash", detail: "pwd", running: true }] },
  ];
  await act(() => {
    renderer = create(createElement("Transcript", null,
  ...
  expect(renderer.root.findAll((node) => node.props.accessibilityLiveRegion === "polite")).toHaveLength(1);
...
it("updates a status phrase without remounting its dot loader", async () => {
  await act(() => { renderer = create(createElement(LiveStatusLine, { phrase: "Thinking" })); });
  ...
  await act(() => renderer.update(createElement(LiveStatusLine, { phrase: "Waiting for you", animating: false })));
