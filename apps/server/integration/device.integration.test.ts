/**
 * Device pane end-to-end verification against a live Graft server.
 *
...
/**
 * Reboot the simulator behind Graft's back, the way `simctl` from a terminal,
...
async function screenshotViaSimctl(udid: string): Promise<Buffer> {
  const path = join(tmpdir(), `graft-device-e2e-${randomUUID()}.png`);
  try {
...
describeE2e("device pane end-to-end", () => {
  ...
  it("boots the target simulator", async () => {
    ...
    expect(booted?.bootSource).toBe("graft");
  }, 180_000);
  ...
  it("keeps input working after the simulator is rebooted outside Graft", async () => {
    if (!target) throw new Error("no target device");
