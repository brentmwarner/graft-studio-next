...
id GraftEmptyTranslatorResponse(void);
...
static id GraftMsgSend0(id target, NSString *selectorName) {
...
static NSString *GraftStringAttribute(id element, NSString *selectorName) {
  id value = GraftMsgSend0(element, selectorName);
  return [value isKindOfClass:NSString.class] ? value : nil;
}

static BOOL GraftBoolAttribute(id element, NSString *selectorName) {
  SEL selector = NSSelectorFromString(selectorName);
...
static id GraftJSONValue(id value) {
  if (value == nil) {
    return NSNull.null;
  }
...
- (instancetype)init {
  ...
  if (self) {
    ...
    _callbackQueue = dispatch_queue_create("dev.graft.device-helper.ax", DISPATCH_QUEUE_CONCURRENT);
    _requestTimeout = 5.0;
  }
  return self;
}
...
- (id)accessibilityTranslationDelegateBridgeCallbackWithToken:(NSString *)token {
  ...
  id (^callback)(id) = ^id(id request) {
    if (device == nil) {
      return GraftEmptyTranslatorResponse();
    }
    ...
    return response ?: GraftEmptyTranslatorResponse();
  };
...
- (nullable NSDictionary *)frontmostTreeWithMaxDepth:(NSInteger)maxDepth error:(NSError **)error {
  if (_translator == nil) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.graft.device-helper.ax"
                                   code:1
...
- (NSDictionary *)serializeElement:(id)element depth:(NSInteger)depth maxDepth:(NSInteger)maxDepth {
  ...
  NSString *role = GraftStringAttribute(element, @"accessibilityRole");
  ...
  NSString *label = GraftStringAttribute(element, @"accessibilityLabel");
  if (label.length > 0) {
    node[@"label"] = label;
  }
  id value = GraftJSONValue(GraftMsgSend0(element, @"accessibilityValue"));
  if (value != NSNull.null) {
    node[@"value"] = value;
  }
  NSString *identifier = GraftStringAttribute(element, @"accessibilityIdentifier");
  if (identifier.length > 0) {
    node[@"identifier"] = identifier;
  }
  NSString *title = GraftStringAttribute(element, @"accessibilityTitle");
  if (title.length > 0) {
    node[@"title"] = title;
  }
  ...
  NSString *subrole = GraftStringAttribute(element, @"accessibilitySubrole");
  ...
  node[@"enabled"] = @(GraftBoolAttribute(element, @"isAccessibilityEnabled"));
  ...
  id rawChildren = GraftMsgSend0(element, @"accessibilityChildren");
