...
typedef union {
  GraftIndigoTouch touch;
  GraftIndigoButton button;
...
typedef struct {
  ...
  unsigned int field3;
  GraftIndigoEvent event;
} GraftIndigoPayload;

typedef struct {
  GraftIndigoMachHeader header;
  unsigned int innerSize;
  unsigned char eventType;
  GraftIndigoPayload payload;
} GraftIndigoMessage;
...
typedef GraftIndigoMessage *(*GraftIndigoButtonFn)(int keyCode, int op, int target);
typedef GraftIndigoMessage *(*GraftIndigoKeyboardFn)(uint32_t usage, int op);
typedef GraftIndigoMessage *(*GraftIndigoMouseFn)(CGPoint *point0, CGPoint *point1, int target,
                                                    int eventType, BOOL extra);
typedef GraftIndigoMessage *(*GraftIndigoArbitraryFn)(int target, uint32_t page, uint32_t usage,
                                                        int op);

BOOL GraftHardwareButtonFromName(NSString *name, GraftHardwareButton *outButton) {
  NSDictionary<NSString *, NSNumber *> *map = @{
    @"home": @(GraftHardwareButtonHome),
    @"lock": @(GraftHardwareButtonLock),
    @"side": @(GraftHardwareButtonSide),
    @"siri": @(GraftHardwareButtonSiri),
    @"volume-up": @(GraftHardwareButtonVolumeUp),
    @"volume-down": @(GraftHardwareButtonVolumeDown),
  };
  ...
  if (outButton != NULL) {
    *outButton = (GraftHardwareButton)found.integerValue;
  }
  return YES;
}

static int GraftButtonSource(GraftHardwareButton button) {
  switch (button) {
    case GraftHardwareButtonHome: return kButtonSourceHome;
    case GraftHardwareButtonLock: return kButtonSourceLock;
    case GraftHardwareButtonSide: return kButtonSourceSide;
    case GraftHardwareButtonSiri: return kButtonSourceSiri;
    case GraftHardwareButtonVolumeUp:
    case GraftHardwareButtonVolumeDown: return -1;
  }
...
static uint32_t GraftConsumerUsage(GraftHardwareButton button) {
  switch (button) {
    case GraftHardwareButtonVolumeUp: return kHIDUsageVolumeIncrement;
    case GraftHardwareButtonVolumeDown: return kHIDUsageVolumeDecrement;
    default: return 0;
  }
}
...
static uint32_t GraftUsageForCharacter(unichar c, BOOL *outShift) {
  BOOL shift = NO;
  uint32_t usage = 0;
...
@implementation GraftHIDBridge {
  id _client;
  dispatch_queue_t _sendQueue;
  GraftIndigoButtonFn _buttonFn;
  GraftIndigoKeyboardFn _keyboardFn;
  GraftIndigoMouseFn _mouseFn;
  GraftIndigoArbitraryFn _arbitraryFn;
...
- (BOOL)attachToDevice:(id)device error:(NSError **)error {
  NSString *developerDir = NSProcessInfo.processInfo.environment[@"GRAFT_DEVELOPER_DIR"];
  ...
  if (kit == NULL) {
    if (error) {
      *error = [NSError errorWithDomain:@"dev.graft.device-helper.hid"
                                   code:1
  ...
  _arbitraryFn = (GraftIndigoArbitraryFn)dlsym(kit, "IndigoHIDMessageForHIDArbitrary");
  _buttonFn = (GraftIndigoButtonFn)dlsym(kit, "IndigoHIDMessageForButton");
  _keyboardFn = (GraftIndigoKeyboardFn)dlsym(kit, "IndigoHIDMessageForKeyboardArbitrary");
  _mouseFn = (GraftIndigoMouseFn)dlsym(kit, "IndigoHIDMessageForMouseNSEvent");
  ...
  if (client == nil) {
    if (error) {
      *error = initError ?: [NSError errorWithDomain:@"dev.graft.device-helper.hid"
                                                code:4
  ...
  _client = client;
  _sendQueue = dispatch_queue_create("dev.graft.device-helper.hid", DISPATCH_QUEUE_SERIAL);
  return YES;
}

- (void)sendMessage:(GraftIndigoMessage *)message {
  ...
  ((void (*)(id, SEL, GraftIndigoMessage *, BOOL, dispatch_queue_t, void (^)(NSError *)))objc_msgSend)(
...
- (void)sendTouchAtX:(double)x y:(double)y down:(BOOL)down {
  ...
  GraftIndigoMessage *seed = _mouseFn(&point, NULL, kTouchTarget, down ? kButtonOpDown : kButtonOpUp, NO);
  if (seed == NULL) {
    [self noteUndelivered];
    return;
  }
  ...
  size_t stride = sizeof(GraftIndigoPayload);
  GraftIndigoMessage *message = calloc(1, sizeof(GraftIndigoMessage) + stride);
  if (message == NULL) {
    free(seed);
    [self noteUndelivered];
    return;
  ...
  memcpy(&message->payload.event.touch, &seed->payload.event.touch, sizeof(GraftIndigoTouch));
  ...
  GraftIndigoPayload *secondPayload = (GraftIndigoPayload *)second;
...
- (void)sendKeyUsage:(uint32_t)usage down:(BOOL)down {
  ...
  GraftIndigoMessage *message = _keyboardFn(usage, down ? kButtonOpDown : kButtonOpUp);
  [self sendMessage:message];
}
...
- (NSInteger)typeText:(NSString *)text {
  ...
  for (NSUInteger index = 0; index < length; index++) {
    ...
    BOOL needsShift = NO;
    uint32_t usage = GraftUsageForCharacter(character, &needsShift);
    if (usage == 0) {
      skipped++;
      continue;
    }
...
- (void)sendButton:(GraftHardwareButton)button down:(BOOL)down {
  int op = down ? kButtonOpDown : kButtonOpUp;
  uint32_t consumerUsage = GraftConsumerUsage(button);
  ...
  [self sendMessage:_buttonFn(GraftButtonSource(button), op, kButtonTargetHardware)];
}

- (void)tapButton:(GraftHardwareButton)button {
