// graft-device-helper — the native side of Graft's Device Pane.
//
...
final class HelperSession {
  ...
  private(set) var hid: GraftHIDBridge?
  private(set) var accessibility: GraftAXBridge?
  ...
  func requireHID() throws -> GraftHIDBridge {
  ...
  func requireAccessibility() throws -> GraftAXBridge {
...
///
/// Every injection path in GraftHIDBridge returns silently when it has no
...
private func withHIDDelivery(_ hid: GraftHIDBridge, _ body: () -> Void) throws {
...
func handle(method: String, params: Params, session: HelperSession) throws -> Any {
  ...
  case "button":
    ...
    var button = GraftHardwareButton.home
    guard GraftHardwareButtonFromName(name, &button) else {
      throw RPCError(
        .invalidParams,
...
let requestQueue = DispatchQueue(label: "dev.graft.device-helper.rpc")
