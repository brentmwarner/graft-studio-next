...
import { CliConfig, makeServerStartupLogData, graftCli, type CliConfigShape } from "./main";
...
let defaultGraftHome = "";
const tempHomes = new Set<string>();

function makeTempHome(prefix = "graft-main-test-"): string {
...
const runCli = (args: ReadonlyArray<string>, env: Record<string, string> = {}) => {
  const program = Command.runWith(graftCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
      ConfigProvider.layer(
        ConfigProvider.fromEnv({
          env: {
            GRAFT_HOME: defaultGraftHome,
            GRAFT_NO_BROWSER: "true",
            ...env,
          },
        }),
      ),
...
it.layer(testLayer)("server CLI command", (it) => {
  it.effect("parses all CLI flags and wires scoped start/stop", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("graft-main-flag-");
  ...
  it.effect("passes the root --home-dir flag to MCP subcommands", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("graft-main-mcp-flag-");
  ...
  it.effect("discovers the persisted runtime origin for the server status subcommand", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("graft-main-status-flag-");
  ...
  it.effect("rejects a discovered endpoint that cannot prove the persisted runtime identity", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("graft-main-status-proof-");
      const stateDir = path.join(flagHome, "userdata");
      fs.mkdirSync(stateDir, { recursive: true, mode: 0o700 });
      fs.chmodSync(stateDir, 0o700);
      fs.writeFileSync(
  ...
  it.effect("reports an unreachable result when no persisted runtime exists", () =>
    Effect.gen(function* () {
      const flagHome = makeTempHome("graft-main-status-missing-");
      const previousExitCode = process.exitCode;
      const output: string[] = [];
      const stdout = vi.spyOn(process.stdout, "write").mockImplementation((chunk) => {
        output.push(String(chunk));
  ...
  it.effect("creates fresh local state directories with private permissions", () =>
    Effect.gen(function* () {
      ...
      const homeDir = makeTempHome("graft-main-private-fresh-");
  ...
  it.effect("repairs permissions for an upgraded local state directory", () =>
    Effect.gen(function* () {
      if (process.platform === "win32") return;
      const homeDir = makeTempHome("graft-main-private-upgrade-");
      const stateDir = path.join(homeDir, "userdata");
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      const envHome = makeTempHome("graft-main-env-");
      ...
      yield* runCli([], {
        ...
        GRAFT_NO_BROWSER: "true",
        GRAFT_AUTH_TOKEN: "env-token",
        GRAFT_DESKTOP_SHUTDOWN_TOKEN: "shutdown-token",
        GRAFT_MIGRATION_DIVERGENCE_CONSENT: "migration-consent",
      });
  ...
  it.effect("consumes desktop shutdown authority before generic child launches", () =>
    Effect.gen(function* () {
      const canonicalKey = "GRAFT_DESKTOP_SHUTDOWN_TOKEN";
      ...
      try {
        ...
        resolvedConfig = null;
        yield* runCli([], { GRAFT_DESKTOP_SHUTDOWN_TOKEN: injectedToken });
  ...
  it.effect("consumes migration divergence consent before generic child launches", () =>
    Effect.gen(function* () {
      const environmentKey = "GRAFT_MIGRATION_DIVERGENCE_CONSENT";
  ...
  it.effect("omits both server authority secrets from startup log data", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        GRAFT_AUTH_TOKEN: "browser-secret",
        GRAFT_DESKTOP_SHUTDOWN_TOKEN: "shutdown-secret",
        GRAFT_MIGRATION_DIVERGENCE_CONSENT: "migration-secret",
      });
  ...
  it.effect("prefers --mode over GRAFT_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over GRAFT_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        GRAFT_NO_BROWSER: "false",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        ...
        ],
        {
          GRAFT_MODE: "desktop",
          GRAFT_NO_BROWSER: "true",
          GRAFT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
          GRAFT_LOG_PROVIDER_EVENTS: "true",
          GRAFT_LOG_WS_EVENTS: "true",
        },
      );
  ...
  it.effect("honors insecure remote opt-in from the environment when the CLI flag is absent", () =>
    Effect.gen(function* () {
      yield* runCli(["--host", "0.0.0.0", "--auth-token", "remote-secret"], {
        GRAFT_ALLOW_INSECURE_REMOTE: "true",
        GRAFT_NO_BROWSER: "true",
      });
    ...
    Effect.gen(function* () {
      yield* runCli(
        [
          ...
          "remote-secret",
          "--public-url",
          "https://graft.example.test",
        ],
        { GRAFT_NO_BROWSER: "false" },
      );

      assert.equal(resolvedConfig?.publicUrl?.origin, "https://graft.example.test");
      assert.equal(openBrowser.mock.calls.length, 1);
      assert.match(
        openBrowser.mock.calls[0]?.[0] ?? "",
        /^https:\/\/graft\.example\.test\/pair#token=/,
      );
    }),
  );
  ...
  it.effect("supports the HTTPS public origin through environment configuration", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        GRAFT_BIND_HOST: "192.168.1.50",
        GRAFT_AUTH_TOKEN: "remote-secret",
        GRAFT_PUBLIC_URL: "https://graft.example.test",
      });
  ...
  it.effect("rejects non-root or non-HTTPS public URLs", () =>
    Effect.gen(function* () {
      for (const publicUrl of ["http://graft.example.test", "https://graft.example.test/app"]) {
        const error = yield* Effect.flip(
  ...
  it.effect("refuses non-loopback exposure without authentication", () =>
    Effect.gen(function* () {
      ...
      assert.match(String(error), /Refusing to bind Graft to non-loopback host 0\.0\.0\.0/);
    }),
  );
  ...
  it.effect("supports CLI and env for bootstrap/provider-log/websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        GRAFT_MODE: "desktop",
        GRAFT_LOG_PROVIDER_EVENTS: "true",
        GRAFT_LOG_WS_EVENTS: "false",
        GRAFT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        GRAFT_NO_BROWSER: "true",
      });
  ...
  it.effect("rejects invalid boolean environment values instead of treating them as absent", () =>
    Effect.gen(function* () {
      const error = yield* Effect.flip(
        runCli([], {
          GRAFT_LOG_PROVIDER_EVENTS: "sometimes",
        }),
      );
