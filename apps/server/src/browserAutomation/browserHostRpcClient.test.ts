...
const TEST_CAPABILITY = "graft-browser-host-client-test-capability-0123456789";

function pipePathForTest(name: string): string {
  return process.platform === "win32"
    ? String.raw`\\.\pipe\graft-${name}-${process.pid}-${crypto.randomUUID()}`
    : path.join("/tmp", `graft-${process.pid}-${crypto.randomUUID().slice(0, 8)}.sock`);
}
...
describe("browser host RPC client", () => {
  ...
  it("prefers the canonical host path and accepts the legacy alias during upgrade", () => {
    expect(
      resolveBrowserHostPipePath({
        GRAFT_BROWSER_HOST_PIPE_PATH: "/tmp/canonical.sock",
        GRAFT_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock",
      }),
    ).toBe("/tmp/canonical.sock");
    expect(resolveBrowserHostPipePath({ GRAFT_BROWSER_USE_PIPE_PATH: "/tmp/legacy.sock" })).toBe(
      "/tmp/legacy.sock",
    );
  });
