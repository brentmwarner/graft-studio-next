...
import { createCachedLoginShellPathReader } from "@graft/shared/loginShellEnvironment";
import {
  expandHomePath as expandHomePathSync,
  resolveGraftHomeDirectory,
} from "@graft/shared/graftHome";
...
export function fixPath(
  ...
  try {
    ...
    // Cached by default: the probe result is persisted under the Graft home and reused
...
export const resolveBaseDir = (raw: string | undefined): Effect.Effect<string> =>
  Effect.succeed(resolveGraftHomeDirectory({ configuredHome: raw }));
