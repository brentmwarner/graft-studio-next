...
import { prewarmChatGptVoiceTranscriptionConnection } from "@graft/shared/chatGptVoiceTranscription";
import {
...
import { decodeSubagentReceiverThreadIds } from "@graft/shared/subagents";
...
import { GRAFT_GATEWAY_HARNESS_POLICY } from "./agentGateway/harnessPolicy.ts";
import {
...
The tools are already callable inside \`functions.exec\`. To open a URL, your first tool call is \`const r = await tools.mcp__graft__browser_open({url: "https://example.com"}); text(r.structuredContent ?? r);\`, substituting the requested URL. No shell commands, skill reads, status checks or inventories are needed. A successful open completes an open-only request.

Use the exact \`tools.mcp__graft__browser_*\` prefix. Available suffixes: ${BROWSER_TOOL_NAMES.map((name) => `\`${name.slice("browser_".length)}\``).join(", ")}.
...
// Maps Graft's simple runtime toggle to Codex thread-level permission overrides.
...
// Graft re-sends turn-level Codex permission overrides, so keep "always allow"
...
export class CodexAppServerManager extends EventEmitter<CodexAppServerManagerEvents> {
  ...
  private readonly graftSkillsDir: string | undefined;
  ...
  constructor(
    ...
    options?: {
      readonly graftSkillsDir?: string;
    ...
    this.graftSkillsDir = options?.graftSkillsDir;
  ...
  // The Graft MCP server rides on the shared overlay config (no secrets),
  ...
  private async buildSessionProcessEnv(
    ...
    if (gatewayBearerToken) {
      env[GRAFT_AGENT_GATEWAY_TOKEN_ENV] = gatewayBearerToken;
    }
    return env;
  }

  // Registers `~/.graft/skills` as a codex skill root so portable skills are
  ...
  private async registerGraftSkillsRoot(context: CodexSessionContext): Promise<void> {
    if (!this.graftSkillsDir) {
      return;
    }
    try {
      await this.sendRequest(context, "skills/extraRoots/set", {
        extraRoots: [this.graftSkillsDir],
      });
    } catch (error) {
      if (!this.isContextRoutable(context)) throw error;
      // Older codex builds (< extra-roots support) keep working; Graft-only
  ...
  async startSession(input: CodexAppServerStartSessionInput): Promise<ProviderSession> {
    ...
    try {
      ...
      await this.registerGraftSkillsRoot(context);
      ...
      if (
        ...
        this.emitLifecycleEvent(
          ...
          "Starting a new Codex thread for a Graft thread that previously had a provider binding.",
        );
        await Effect.logWarning(
          "codex app-server starting a fresh thread for a previously bound Graft thread",
          {
            threadId,
  ...
  private async handleServerRequest(
    ...
    if (isUserInputRequest) {
      ...
      const detail = "Codex asked a question Graft could not render, so it was declined.";
