...
function scratchCacheRoot(homeDirectory: string, platform: NodeJS.Platform): string {
  if (platform === "darwin") return path.join(homeDirectory, "Library", "Caches", "graft");
  if (platform === "win32") {
    return path.join(homeDirectory, "AppData", "Local", "Graft", "Cache");
  }
  return path.join(homeDirectory, ".cache", "graft");
}
...
export function resolveScratchWorkspacesRoot(
  ...
  const ownerContainer = path.join(
    ...
    `.graft-${scratchOwnerSegment(homeDirectory)}`,
  );
