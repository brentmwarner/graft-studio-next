...
import {
  ...
  takeGraftHarnessPolicyForSession,
} from "./harnessPolicy.ts";

describe("Graft harness policy", () => {
  it("defers duplicate automation authoring text while preserving tool routing and run rules", () => {
    const inline = renderGraftHarnessPolicy({ gatewayControlAvailable: true });
    const deferred = renderGraftHarnessPolicy({
      gatewayControlAvailable: true,
      automationAuthoring: "tool-descriptions",
    });
    assert.equal(deferred, inline.replace(`${AUTOMATION_AUTHORING_GUIDANCE}\n`, ""));
    assert.include(deferred, "graft_create_automation");
    assert.include(deferred, "graft_view_automation");
    assert.include(deferred, "graft_report_automation_result");
  });
  it("includes honest completion evidence and opt-in delegated E2E testing", () => {
    const policy = renderGraftHarnessPolicy({ gatewayControlAvailable: true });
    for (const text of [
      "completion report",
      "artifactPath",
      "![Result description]",
    ...
    assert.notInclude(
      renderGraftHarnessPolicy({ gatewayControlAvailable: false }),
      "browser_screenshot({kind:'proof'})",
    );
  });
  it("identifies Graft and explains exact batch coordination when MCP is available", () => {
    const policy = renderGraftHarnessPolicy({ gatewayControlAvailable: true });
    assert.include(policy, GRAFT_HARNESS_POLICY_MARKER);
    assert.include(policy, "Graft is the host and harness");
    assert.include(policy, "one exact graft_create_threads plan");
    assert.include(policy, "before returning an operationId");
    assert.include(policy, "graft_wait_for_threads");
    assert.include(policy, "graft_set_thread_pull_request");
    ...
    assert.include(policy, "do not create Graft threads");
  ...
  it("asks agents to emit known absolute file URLs instead of invented relative links", () => {
    const gateway = renderGraftHarnessPolicy({ gatewayControlAvailable: true });
    const identityOnly = renderGraftHarnessPolicy({ gatewayControlAvailable: false });
  ...
  it("never advertises gateway mutation to providers without scoped MCP", () => {
    const policy = renderGraftHarnessPolicy({ gatewayControlAvailable: false });
    assert.include(policy, "Graft MCP control is unavailable");
    assert.notInclude(policy, "one exact graft_create_threads plan");
  });

  it("delivers a private host-context block once per provider session", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    assert.include(
      takeGraftHarnessPolicyForSession(state, { gatewayControlAvailable: true }) ?? "",
      "<graft_host_context>",
    );
    assert.isNull(takeGraftHarnessPolicyForSession(state, { gatewayControlAvailable: true }));
  });
  ...
  it("delivers once on fresh/load/fork sessions for every scoped MCP provider", () => {
    for (const provider of ["antigravity", "cursor", "grok", "droid", "opencode", "pi"] as const) {
      for (const lifecycle of ["fresh", "load", "fork"] as const) {
        ...
        assert.include(first, GRAFT_HARNESS_POLICY_MARKER, `${provider}/${lifecycle}`);
        assert.include(first, "Use the graft_* tools", `${provider}/${lifecycle}`);
        assert.isNull(
          takeGraftHarnessPolicyForProviderSession(state, {
            provider,
            scopedGatewayConnectionAvailable: true,
          }),
          `${provider}/${lifecycle}`,
  ...
  it("keeps OpenCode and Pi identity-only until scoped setup succeeds", () => {
    for (const provider of ["opencode", "pi"] as const) {
      ...
      assert.include(text, GRAFT_HARNESS_POLICY_MARKER, provider);
      assert.include(text, "Graft MCP control is unavailable", provider);
      assert.notInclude(text, "one exact graft_create_threads plan", provider);
    }
  });
  ...
  it("keeps the gateway policy below its prompt budget", () => {
    assert.isAtMost(renderGraftHarnessPolicy({ gatewayControlAvailable: true }).length, 6_000);
  });
