/**
 * Stdio-to-HTTP proxy script for the Graft agent gateway.
 *
...
const STDIO_PROXY_SCRIPT = `// Graft agent gateway stdio<->HTTP MCP proxy (generated file, do not edit).
const url = process.env.GRAFT_AGENT_GATEWAY_URL;
let token = process.env.GRAFT_AGENT_GATEWAY_TOKEN;
let bootstrapToken = process.env.GRAFT_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
...
function localInactiveResponse(message) {
  ...
  // CLI launched outside Graft therefore sees a valid empty MCP server
  // instead of a noisy failed integration. Graft-managed processes receive
    ...
    return [
      {
        ...
        result: {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: "graft", version: "1.0.0" },
        },
      },
    ];
  }
  ...
  return [
    {
      jsonrpc: "2.0",
      id,
      error: { code: -32601, message: "Graft is not active for this Antigravity session." },
    },
  ];
}
...
async function resolveToken() {
  ...
  if (!tokenResolution) {
    ...
    tokenResolution = (async () => {
      ...
      if (!response.ok) {
        throw new Error("Graft gateway bootstrap failed with HTTP " + response.status);
      }
      const payload = await response.json();
      if (!isRecord(payload) || typeof payload.bearerToken !== "string") {
        throw new Error("Graft gateway bootstrap returned an invalid response");
      }
      token = payload.bearerToken;
      bootstrapToken = undefined;
      delete process.env.GRAFT_AGENT_GATEWAY_BOOTSTRAP_TOKEN;
      return token;
    })().finally(() => {
    ...
    return [
      {
        jsonrpc: "2.0",
        id,
        error: { code: -32603, message: "Graft gateway request failed: " + String(error) },
      },
    ];
  }
}
