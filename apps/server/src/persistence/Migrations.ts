...
/**
 * A tracker identity that a *released* Graft build wrote for a migration whose
 ...
 * does the migration Graft registers at `historicalId` *today* still need to
 ...
 * once the imported tracker's high-water mark reaches Graft's latest ID,
 * every Graft migration is skipped silently and startup crashes on missing
 ...
 *
 * Instead, compare the recorded (id, name) pairs against Graft's lineage and
 ...
 *
 * Divergences caused by Graft renumbering one of its *own* released
...
export const reconcileMigrationLineage = Effect.gen(function* () {
  ...
  // A released Graft build may have recorded a migration under an ID it no
  ...
  if (aliasRepairs.length > 0) {
    yield* Effect.logInfo(
      "Migration tracker records a renumbered Graft migration; repairing tracker metadata in place",
    ).pipe(
      Effect.annotateLogs({
  ...
  if (diverged === undefined) {
    ...
    // a newer Graft build. Continuing would expose it to stale writable
  ...
  yield* Effect.logWarning(
    "Migration tracker diverges from the Graft lineage (legacy import); re-running migrations from the divergence point",
