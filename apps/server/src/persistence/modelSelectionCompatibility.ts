...
import { MODEL_OPTIONS_BY_PROVIDER } from "@graft/contracts";
...
// Imported instance ids may be runtime names rather than Graft provider literals.
...
export function normalizePersistedModelSelection(input: unknown): unknown {
  ...
  // Newer Graft writes provider-less selections as { instanceId, model } and
  // option rows as [{ id, value }]; Graft stores canonical provider/options objects.
