...
const checkpoint: OrchestrationCheckpointSummary = {
  ...
  checkpointRef: CheckpointRef.makeUnsafe("refs/graft/test"),
  status: "ready",
  files: [{ ...summary, kind: "modified" }],
...
describe("mobile patch files", () => {
  it("preserves both line-number sequences, context gaps, and syntax text", async () => {
    ...
    expect(
      file.hunks?.[0]?.lines.map(({ kind, oldLine, newLine }) => [kind, oldLine, newLine]),
    ).toEqual([
      ["context", 20, 20],
      ["context", 21, 21],
      ["deletion", 22, undefined],
      ["addition", undefined, 22],
      ["addition", undefined, 23],
      ["context", 23, 24],
    ]);
    expect(file.hunks?.[1]?.collapsedBefore).toBe(9);
    expect(
      file.hunks?.[0]?.lines[3]?.tokens
    ...
    expect(
      file.hunks?.[0]?.lines[0]?.tokens?.some((token) => token.lightColor && token.darkColor),
    ).toBe(true);
  });
  it("emphasizes the changed characters of an isolated replacement", async () => {
    const file = await Effect.runPromise(
      mobilePatchFile(
        summary,
        patch
          .replace('+  "@graft/mobile-contract": "workspace:*"\n', "")
    ...
    expect(
      file.hunks?.[0]?.lines
        ...
        ?.tokens?.filter((token) => token.changed)
        .map((token) => token.text)
        .join(""),
    ).toBe(",");
  });
  it("handles added files without inventing old line zero", async () => {
    const file = await Effect.runPromise(
      mobilePatchFile(
        { path: "hello.ts", status: "added" },
        "diff --git a/hello.ts b/hello.ts\nnew file mode 100644\n--- /dev/null\n+++ b/hello.ts\n@@ -0,0 +1,2 @@\n+export const hello = 1;\n+\n",
      ),
    );
    expect(file.hunks?.[0]?.lines.map((line) => [line.oldLine, line.newLine])).toEqual([
      [undefined, 1],
      [undefined, 2],
    ]);
    expect(
      GraftDiffSummarySchema.safeParse({ id: "a", threadId: "a", updatedAt: 1, files: [file] })
        .success,
    ).toBe(true);
  });
  ...
  it("does not compute patches for summaries or paths outside the checkpoint", async () => {
    ...
    const layer = Layer.succeed(CheckpointDiffQuery, {
      getTurnDiff: query,
      getFullThreadDiff: query,
    });
    await Effect.runPromise(loadMobileDiff("t", checkpoint).pipe(Effect.provide(layer)));
    await Effect.runPromise(
      loadMobileDiff("t", checkpoint, "../private").pipe(Effect.provide(layer)),
    );
  ...
  it("reads the matching turn with whitespace changes included", async () => {
    ...
    const result = await Effect.runPromise(
      loadMobileDiff("t", checkpoint, "package.json").pipe(Effect.provide(layer)),
    );
