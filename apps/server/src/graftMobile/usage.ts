...
export function toMobileContextUsage(activities: ReadonlyArray<OrchestrationThreadActivity>): GraftContextUsage | undefined {
  ...
  const measurement = activities.findLast(
    (activity) => activity.kind === "context-window.updated" && activity.createdAt === snapshot.updatedAt,
  );
  if (!measurement) return undefined;
  const payload = measurement.payload;
  const rawTokens = payload !== null && typeof payload === "object" && !Array.isArray(payload) && "usedTokens" in payload
    ? payload.usedTokens
    : undefined;
  const hasTokenCount = typeof rawTokens === "number" && Number.isFinite(rawTokens) && rawTokens >= 0;
  return {
...
export function toMobileAllowance(providerId: string, snapshot?: ServerProviderUsageSnapshot): GraftProviderAllowance {
  ...
  return {
    ...
    limits: snapshot.limits.flatMap((limit) => {
      ...
      const label = weekly && !/week|7.day/i.test(limit.window) ? `Weekly · ${limit.window}` : limit.window;
      return [{
        label,
