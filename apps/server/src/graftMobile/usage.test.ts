...
describe("mobile context usage", () => {
  ...
  it("does not present capacity alone as measured zero usage", () => {
    expect(
      toMobileContextUsage([activity("context-window.configured", { maxTokens: 200000 })]),
    ).toBeUndefined();
  });
  it("invalidates context after compaction until a new measurement", () => {
    ...
    expect(
      toMobileContextUsage([
        previous,
        compacted,
        activity("context-window.updated", { usedPercent: 10 }, "2026-09-14T12:02:00.000Z"),
      ]),
  ...
  it("does not invent a token ratio when only percentage is reported", () => {
    expect(toMobileContextUsage([activity("context-window.updated", { usedPercent: 42 })])).toEqual(
      { percent: 42, tokensUsed: 0, tokensMax: 0, source: "measured" },
    );
  });
  it("does not turn a percentage plus capacity into a fictitious zero token count", () => {
    expect(
      toMobileContextUsage([
        activity("context-window.updated", { usedPercent: 42, maxTokens: 200000 }),
      ]),
...
describe("mobile provider allowance", () => {
  ...
  it("keeps unsupported, sign-in-required, and stale results honest", () => {
    expect(toMobileAllowance("pi")).toEqual({
      providerId: "pi",
      status: "unsupported",
      stale: false,
      limits: [],
    });
    expect(toMobileAllowance("codex", provider({ status: "needs-auth" }))).toMatchObject({
      status: "needs-auth",
      limits: [],
    });
    expect(
      toMobileAllowance(
        "codex",
        provider({ stale: true, limits: [{ window: "Weekly", usedPercent: 20 }] }),
      ),
    ).toMatchObject({ stale: true, limits: [{ remainingPercent: 80 }] });
    expect(
      toMobileAllowance(
        "codex",
        provider({ status: "error", limits: [{ window: "Weekly", usedPercent: 20 }] }),
      ),
  ...
  it("omits unreported percentages and never invents a weekly window", () => {
    expect(
      toMobileAllowance(
        "codex",
        provider({ limits: [{ window: "5h", usedPercent: 20 }, { window: "Weekly" }] }),
      ).limits,
