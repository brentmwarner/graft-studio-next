...
function callbackRequest(options: CallbackRequestOptions): Promise<LoopbackResponse> {
  ...
  return new Promise((resolve, reject) => {
    ...
    request.setTimeout(options.timeoutMs ?? 1000, () => {
...
describe("GraftAccountLoginService", () => {
  it("keeps a validated callback open while relay registration is slow", async () => {
    ...
    const { port, state } = parseSignInUrl(harness.openedUrls);
    const response = await callbackRequest({
      port,
      body: validCallbackBody(state),
      timeoutMs: 8_000,
    });
    expect(response.status).toBe(200);
    expect(harness.completions).toEqual([
      { ok: true, email: "person@example.com", name: "Example Person" },
    ]);
  }, 10_000);
