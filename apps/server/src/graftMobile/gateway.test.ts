...
it("dispatches mobile attachments with the authenticated session and real interaction mode", async () => {
  ...
  const current = {
    id: "thread-1", projectId: "project-1", title: "Test",
    modelSelection: { provider: "codex", model: "gpt-5", options: { reasoningEffort: "high" } },
    interactionMode: "debug", runtimeMode: "approval-required",
  ...
  for (const additions of [{ attachments: [attachment], interactionMode: "plan", fastMode: true }, {}]) {
    const command = GraftMobileCommandSchema.parse({ type: "turn.start", threadId: "thread-1", text: "Review", ...additions });
    await Effect.runPromise(executeMobileCommand(makeGraftMobileGatewayState(), "mobile-command", command, context).pipe(Effect.provide(layer)));
  }
  expect(dispatch.mock.calls[0]).toEqual([
    expect.objectContaining({
      type: "thread.turn.start", interactionMode: "plan", runtimeMode: "approval-required",
      message: expect.objectContaining({ text: "Review", attachments: [attachment] }),
      modelSelection: { provider: "codex", model: "gpt-5", options: { reasoningEffort: "high", fastMode: true } },
    }),
    context,
  ]);
  expect(dispatch.mock.calls[1]).toEqual([
    expect.objectContaining({ interactionMode: "debug", message: expect.objectContaining({ attachments: [] }), modelSelection: current.modelSelection }),
    context,
  ]);
});
