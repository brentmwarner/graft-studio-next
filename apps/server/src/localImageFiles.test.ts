...
describe("resolveAllowedLocalPreviewFile", () => {
  it("allows images inside the current workspace", async () => {
    const workspace = makeTempDir("graft-image-workspace-");
  ...
  it("allows images inside Codex generated_images without a cwd", async () => {
    const codexHome = makeTempDir("graft-codex-home-");
  ...
  it("allows images written to the GRAFT_HOME codex-home-overlay generated_images root", async () => {
    // Codex app-server is launched with CODEX_HOME pointing at a Graft overlay
    // directory (see resolveGraftCodexHomeOverlayPath). Generated images therefore
    // live under <GRAFT_HOME>/codex-home-overlay/generated_images/<thread>/<call>.png,
    ...
    const graftHome = path.join(fakeRoot, "graft", "runtime");
  ...
  it("allows PDFs inside the current workspace", async () => {
    const workspace = makeTempDir("graft-pdf-workspace-");
    writeFileSync(path.join(workspace, ".git"), "gitdir: .git");
  ...
  it("allows PDFs inside a per-thread scratch workspace without a cwd", async () => {
    ...
    // <tmpdir>/graft-codex-workspaces/<threadId>; files agents create there
    // are workspace-equivalent, so documents must be servable from that root.
    const scratchRoot = path.join(os.tmpdir(), "graft-codex-workspaces");
  ...
  it("allows PDFs inside the configured private scratch root without a cwd", async () => {
    const privateTempRoot = makeTempDir("graft-private-scratch-");
    const scratchRoot = path.join(privateTempRoot, "graft-codex-workspaces");
  ...
  it("rejects PDFs outside the workspace even under the temp-dir image roots", async () => {
    ...
    const tempDir = makeTempDir("graft-pdf-outside-");
  ...
  it("still allows images under the temp-dir roots without a workspace", async () => {
    const tempDir = makeTempDir("graft-image-tmp-root-");
