import type { ExternalMcpCapability } from "@graft/contracts";
...
export function buildExternalMcpOverviewNextSteps(
  ...
  return [
    ...
    ...(capabilities.has("tasks:create") ? ["Create work with graft_create_task."] : []),
    ...(capabilities.has("tasks:wait") ? ["Follow permitted work with graft_wait_for_task."] : []),
    ...(capabilities.has("tasks:read")
      ? ["Read permitted task results with graft_read_task."]
      : []),
  ];
}
