...
export class IosSimulatorBackend implements DeviceBackend {
  ...
  async availability(): Promise<DeviceAvailability> {
    ...
    steps.push({
      ...
      label: "Build the Graft device helper",
      done: helperBuilt,
  ...
  async screenshot(
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "graft-device-"));
   ...
   * Graft, SimulatorKit dropped the connection) the injection silently
  ...
  private async attachedHelper(
    ...
    try {
      ...
      // A device can also be rebooted outside Graft (Simulator.app, or simctl
