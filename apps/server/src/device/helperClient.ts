 ...
 *   already the `@graft/contracts` device-frame envelope, so this module only
...
import { decodeDeviceFrame } from "@graft/shared/deviceFrame";
...
export class HelperClient {
  ...
  async startStream(udid: string, onFrame: (frame: DeviceStreamFrame) => void): Promise<void> {
    ...
    const directory = await mkdtemp(path.join(tmpdir(), "graft-device-frames-"));
