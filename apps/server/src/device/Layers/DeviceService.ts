 ...
 * directory so both land in the same place under a custom GRAFT_HOME.
 *
...
function defaultBootOwnershipPath(): string {
  const baseDir = resolveGraftHomeDirectory();
          ...
          if (reclaimed.length > 0) {
            console.info(
              `[device] shut down ${reclaimed.length} simulator(s) left booted by a previous ` +
                `Graft run: ${reclaimed.join(", ")}`,
            );
          }
        });
      }

      // App quit shuts down every simulator Graft booted and leaves the
