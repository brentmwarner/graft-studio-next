...
function makeBackend() {
  ...
  const backend = new IosSimulatorBackend({
    platform: "darwin",
    helperCacheRoot: "/tmp/graft-device-test-cache",
  ...
  Object.defineProperty(backend, "compileHelperIfNeeded", {
    value: async () => "/tmp/graft-device-test-cache/graft-device-helper",
  });
...
describe("simulator reboot", () => {
  it("re-attaches after a Graft-driven shutdown and reboot", async () => {
  ...
  it("recovers when the device was rebooted outside Graft", async () => {
    const { backend, helper } = makeBackend();
    await backend.tap(DEVICE, 10, 10);
