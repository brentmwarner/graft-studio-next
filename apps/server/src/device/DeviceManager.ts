 ...
 * - The Graft boot cap (`DEVICE_GRAFT_BOOT_LIMIT`). Boot past the cap is
...
/** How long a Graft-booted device stays up with no thread attached. */
...
export interface DeviceManagerOptions {
  ...
  /** Remembers Graft's boots across a crash; defaults to remembering nothing. */
...
export class DeviceManager {
  ...
  private readonly graftBooted = new Set<string>();
  ...
  constructor(options: DeviceManagerOptions) {
    ...
    this.bootLimit = options.bootLimit ?? DEVICE_GRAFT_BOOT_LIMIT;
  ...
  private async recordBootOwnership(): Promise<void> {
    await this.bootOwnership.write([...this.graftBooted]).catch(() => undefined);
  }
  ...
  async graftBootedDevices(): Promise<readonly DeviceDescriptor[]> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    this.reconcileGraftBooted(devices);
    return devices
      .filter((device) => this.graftBooted.has(device.udid))
   ...
   *
   * The set is Graft's own bookkeeping, but the simulators are not Graft's to
   ...
   */
  private reconcileGraftBooted(devices: readonly DeviceDescriptor[]): void {
    const running = new Set(
      devices
        .filter((device) => device.state === "booted" || device.state === "booting")
        .map((device) => device.udid),
    );
    for (const udid of this.graftBooted) {
      if (running.has(udid)) continue;
      this.graftBooted.delete(udid);
  ...
  async boot(udid: string): Promise<DeviceBootResult> {
    const devices = await this.backend.listDevices({ includeShutdown: true }).catch(() => []);
    // Devices that stopped without Graft doing it still held their slots, so
    ...
    // Graft from accumulating simulators, not to limit what the user watches.
    if (known?.state === "booted") {
      return { kind: "booted", device: this.describe(known) };
    }
    if (this.graftBooted.size >= this.bootLimit) {
      return {
        kind: "boot-limit-reached",
        limit: this.bootLimit,
        graftBooted: await this.graftBootedDevices(),
      };
    }
    ...
    // exists to stop Graft accumulating multi-gigabyte simulators would be
    // exceeded by however many requests arrived inside that window.
    this.graftBooted.add(udid);
    ...
    return { kind: "booted", device: { ...device, bootSource: "graft" } };
  }
  ...
  /**
   * App quit: shut down everything Graft booted, leave the user's devices
  ...
  async dispose(): Promise<void> {
    ...
    const booted = Array.from(this.graftBooted);
  ...
  private describe(device: DeviceDescriptor): DeviceDescriptor {
    const bootSource = this.graftBooted.has(device.udid) ? "graft" : device.bootSource;
   ...
   * the device down or start the idle countdown if Graft booted it.
   *
   * A switch shuts down immediately rather than waiting out the idle window.
   * The cap is three Graft-booted devices, and each one costs a couple of GB
  ...
  private async releaseDevice(
    ...
    if (!this.graftBooted.has(udid)) return;
    if (reason === "switched") {
      this.clearIdleTimer(udid);
  ...
  private async shutdownIfStillIdle(udid: string): Promise<void> {
    ...
    if (this.isAttachedAnywhere(udid) || !this.graftBooted.has(udid)) return;
    await this.shutdown(udid).catch(() => undefined);
  }
