...
// Purpose: Build a ZIP archive that exports a single Graft thread so a user
...
// The export-eligibility guard lives in @graft/shared/threadExport so the
...
export function threadArchiveFileName(input: {
  ...
  return `graft-thread-${slugifyTitle(input.title)}-${dateBucket}.zip`;
}
