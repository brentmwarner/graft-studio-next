...
import type { ProviderRuntimeEvent } from "@graft/contracts";
...
describe("resolveCodexGeneratedImagesRoot(s)", () => {
  ...
  it("returns the overlay generated_images directory as the active write root by default", () => {
    process.env.GRAFT_HOME = "/graft-test/runtime";
    assert.equal(
      resolveCodexGeneratedImagesRoot("/codex-test/.codex"),
      path.join("/graft-test/runtime", "codex-home-overlay", "generated_images"),
    );
  });
  ...
  it("collapses to a single root when overlay equals source", () => {
    ...
    // The overlay falls under `<dirname(source)>/.graft/runtime/codex-home-overlay`,
    ...
    const homePath = "/runtime/.graft/runtime/codex-home-overlay";
