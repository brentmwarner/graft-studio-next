...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "GraftLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  const server: ServerLocalServerProcess = {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "Graft cannot signal this process." }),
  };
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "Graft cannot stop this process.",
    };
  }
