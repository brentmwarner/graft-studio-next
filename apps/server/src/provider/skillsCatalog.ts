...
//          walking) plus the unified cross-provider skills catalog backing Graft
//          portable skills. Aggregates `~/.graft/skills` with every provider-native
...
//          mergeSkillsIntoCatalog, filterDisabledSkills, ensureGraftSkillsDir

import * as fs from "node:fs/promises";
import * as nodePath from "node:path";

import type { ProviderKind, ProviderSkillDescriptor } from "@graft/contracts";
...
export interface SkillsCatalogDiscoveryInput {
  ...
  /** Graft base dir (usually `~/.graft`); skills live in `{base}/skills`. */
...
export interface SkillsCatalogRootInput extends SkillsCatalogDiscoveryInput {
  /** Native provider scans can opt out; the catalog itself always includes Graft. */
  readonly includeGraftRoot?: boolean;
}
...
const ensuredGraftSkillsDirs = new Set<string>();

export function clearSkillsCatalogCacheForTests(): void {
  skillsCatalogCache.clear();
  skillsCatalogInflight.clear();
  ensuredGraftSkillsDirs.clear();
}

export function graftSkillsDir(graftBaseDir: string): string {
  return nodePath.join(graftBaseDir, "skills");
}

// Creates the portable skills folder on first use so users have a drop-in target.
export async function ensureGraftSkillsDir(graftBaseDir: string): Promise<string> {
  const dir = graftSkillsDir(graftBaseDir);
  if (ensuredGraftSkillsDirs.has(dir)) {
    return dir;
  }
  try {
    await fs.mkdir(dir, { recursive: true });
    ensuredGraftSkillsDirs.add(dir);
  } catch {
...
const SKILL_ORIGIN_ROOTS = {
  graft: {
    homeRoots: (input) => [graftSkillsDir(input.graftBaseDir)],
    projectRootNames: [".graft"],
  },
  codex: {
    // Keep Graft's existing Codex-local root. Official Codex discovery uses
...
// Native copies first so an agent keeps using its own skill, then Graft as the
...
function orderedOriginsForProvider(
  provider: ProviderKind | null | undefined,
  includeGraftRoot = true,
  includeRemainingOrigins = true,
): SkillsHomeOrigin[] {
  const preferred = preferredOriginsForProvider(provider);
  const ordered = [...preferred];
  if (includeGraftRoot && !ordered.includes("graft")) {
    ordered.push("graft");
  }
  if (!includeRemainingOrigins) {
    return ordered.filter((origin) => includeGraftRoot || origin !== "graft");
  }
  for (const origin of HOME_ORIGIN_ORDER) {
    if (!includeGraftRoot && origin === "graft") {
      continue;
    }
...
export function skillsCatalogRoots(input: SkillsCatalogRootInput): SkillRoot[] {
  return rootsForOrderedOrigins(
    input,
    orderedOriginsForProvider(input.provider, input.includeGraftRoot !== false),
  );
}
...
export async function discoverSkillsCatalog(
  ...
  const scan = (async () => {
    await ensureGraftSkillsDir(input.graftBaseDir);
    const roots = [
