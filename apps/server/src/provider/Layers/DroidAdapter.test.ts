...
import {
  ...
  takeDroidGraftHarnessPolicyTextPart,
} from "./DroidAdapter.ts";

describe("Droid Graft harness policy", () => {
  it("delivers private scoped host context once", () => {
    const state: { harnessPolicyDelivered?: boolean } = {};
    expect(takeDroidGraftHarnessPolicyTextPart(state, true)?.text).toContain(
      GRAFT_HARNESS_POLICY_MARKER,
    );
    expect(takeDroidGraftHarnessPolicyTextPart(state, true)).toBeNull();
  });
});
