...
export const takeDroidGraftHarnessPolicyTextPart = (
...
const DROID_ACP_DEBUG_ENV = "GRAFT_DROID_ACP_DEBUG";
...
// override with GRAFT_DROID_TURN_IDLE_TIMEOUT_MS when a workload needs longer.
const DROID_TURN_IDLE_TIMEOUT_MS = resolveAcpTurnIdleTimeoutMs({
  envVar: "GRAFT_DROID_TURN_IDLE_TIMEOUT_MS",
  defaultMs: 600_000,
});
...
const DROID_PLAN_MODE_PROMPT_PREFIX = [
  "Graft Droid plan mode is active.",
...
interface DroidSessionContext {
  ...
  /** Turns cancelled by Graft only because their Plan proposal was captured. */
  ...
  return Effect.gen(function* () {
        ...
        Effect.gen(function* () {
          ...
          if (resumeSessionId !== undefined && started.sessionSetupMethod === "new") {
            return yield* new ProviderAdapterRequestError({
              ...
              detail:
                "Droid could not resume the requested native session. Graft refused the fresh fallback to avoid silently losing conversation context.",
            });
          }
    ...
    const startDroidTurn = (
      ...
      Effect.gen(function* () {
        ...
        const harnessPolicy = takeDroidGraftHarnessPolicyTextPart(
          ctx,
      ...
      Effect.gen(function* () {
        ...
        const forkRuntime = (runtime: AcpSessionRuntimeShape) =>
          forkViaAcpRuntime({
            ...
            targetCwd,
            unsupportedIssue:
              "This Droid ACP version does not advertise session/fork; Graft will rebuild the fork from its retained transcript.",
        ...
        if (activeSource?.activeTurnId !== undefined) {
          return yield* new ProviderAdapterValidationError({
            ...
            operation: "forkThread",
            issue:
              "The source Droid session has a turn in flight; Graft will rebuild the fork from its retained transcript.",
          });
        }
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Graft Model Discovery",
          });
          yield* runtime.start();
      ...
      discoveryLock.withPermits(1)(
        Effect.gen(function* () {
          ...
          const runtime = yield* makeDroidDiscoveryRuntime({
            ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
            cwd,
            clientName: "Graft Command Discovery",
          });
          yield* runtime.start();
