// FILE: debugMode.ts
// Purpose: Applies Graft's provider-independent evidence-first Debug instructions.
// Layer: Provider prompt policy

import type { ProviderInteractionMode } from "@graft/contracts";

export const PROVIDER_DEBUG_MODE_PROMPT_PREFIX = `<graft_debug_mode>
You are operating in Graft Debug mode. Diagnose the reported defect using this evidence-first loop: observe -> reproduce -> investigate -> fix -> verify.
...
- Do not imply Graft can observe external actions. If browser state, terminal output, logs, or another required signal is inaccessible, ask the user for that evidence.
