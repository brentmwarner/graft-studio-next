...
describe("createDevinSessionConfig", () => {
  it("merges user MCP entries without mutating the source and keeps credentials paths visible", async () => {
    ...
    expect(generated.mcpServers.graft).toMatchObject({
      command: process.execPath,
      args: ["/proxy.mjs"],
      env: {
        GRAFT_AGENT_GATEWAY_URL: "http://127.0.0.1:3773/mcp",
        GRAFT_AGENT_GATEWAY_BOOTSTRAP_TOKEN: "spent-one-shot-bootstrap",
      },
      transport: "stdio",
    });
  ...
  it("rejects a reserved graft collision without changing user config", async () => {
    const input = await makeInput();
    const source = path.join(input.env.XDG_CONFIG_HOME, "devin", "mcp_config.json");
    const original = JSON.stringify({ mcpServers: { graft: { command: "user-owned" } } });
    await writeFile(source, original);
    await expect(createDevinSessionConfig(input)).rejects.toThrow("reserved 'graft'");
    expect(await readFile(source, "utf8")).toBe(original);
  });
  ...
  it("isolates the Windows MCP config through APPDATA", async () => {
    ...
    expect(JSON.parse(await readFile(config.configPath, "utf8")).mcpServers).toMatchObject({
      ...
      graft: { transport: "stdio" },
    });
    expect(await readFile(sourceConfig, "utf8")).not.toContain("graft");
  });
});
