...
describe("managed checkpoint refs", () => {
  ...
  it("creates canonical Graft refs", () => {
    expect(checkpointRefForThreadTurn(threadId, 4)).toMatch(/^refs\/graft\/checkpoints\//);
  });
  ...
  it("recognizes a structurally valid persisted ref for the same thread", () => {
    ...
    const historical = canonical.replace("refs/graft/", "refs/historical/");
