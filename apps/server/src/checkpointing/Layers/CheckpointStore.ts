  ...
  const captureCheckpointOnce: CheckpointStoreShape["captureCheckpoint"] = (input) =>
    Effect.gen(function* () {
      ...
      yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "graft-fs-checkpoint-" }),
        (tempDir) =>
          Effect.gen(function* () {
            ...
            const commitEnv: NodeJS.ProcessEnv = {
              ...
              GIT_AUTHOR_EMAIL: "graft@users.noreply.github.com",
              GIT_COMMITTER_NAME: "Graft",
              GIT_COMMITTER_EMAIL: "graft@users.noreply.github.com",
            };
            ...
            const message = `Graft checkpoint ref=${input.checkpointRef}`;
  ...
  const reverseCheckpointDiff: CheckpointStoreShape["reverseCheckpointDiff"] = (input) =>
    Effect.gen(function* () {
      ...
      return yield* Effect.acquireUseRelease(
        fs.makeTempDirectory({ prefix: "graft-checkpoint-undo-" }),
        (tempDir) =>
          Effect.gen(function* () {
