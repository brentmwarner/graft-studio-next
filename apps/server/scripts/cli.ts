...
} from "../../../scripts/lib/bundled-server-dependencies.ts";
...
const stageDistributionPackage = Effect.fn("stageDistributionPackage")(function* (
  ...
  yield* Effect.try({
    try: () => verifyBundledServerOutput(path.join(serverDir, "dist")),
    catch: (cause) =>
      new CliError({
  ...
  const pkg = {
    ...
    dependencies: resolveBundledServerDependencies(
...
const packCmd = Command.make(
  ...
  (config) =>
    Effect.gen(function* () {
      ...
      const tarballPath = path.join(outDir, `graft-server-${version}.tar.gz`);
      yield* runCommand(
...
).pipe(Command.withDescription("Produce a graft-server-<version>.tar.gz from the staged package."));
...
const cli = Command.make("cli").pipe(
  Command.withDescription("Graft server build & publish CLI."),
