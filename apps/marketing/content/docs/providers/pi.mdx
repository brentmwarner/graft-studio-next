---
title: Pi
description: Install, authenticate, verify, and run the Pi coding agent through Graft.
---

Graft integrates the Pi coding-agent runtime and its model registry. Pi owns provider credentials, models, extensions, skills, commands, and session behavior; Graft supplies the durable task, working environment, transcript, review surfaces, Agent Gateway connection, and Git lifecycle around it.
...
Do not commit credentials or paste them into Graft tasks. Pi normally stores its user state under `~/.pi/agent/`.
...
This verifies the package, provider credentials, model registry, and working directory before Graft is involved.
...
Open provider settings and confirm that Graft detects `pi`.
...
Use Pi’s own `/login`, environment variables, or supported auth file. Graft reads the resulting model availability rather than duplicating the secret.
...
In Graft 0.8.1 and later, the Anthropic catalog repair keeps Claude Fable 5.1 selectable even when Pi's authenticated upstream catalog predates that model entry. Fable 5.1 uses always-on thinking with Low through Max effort and no fast-mode lane.
...
- User-input requests surfaced in the Graft conversation
...
Pi extensions and packages execute with the permissions of your user account. Review third-party code before enabling it in a repository Graft will operate.
...
You can also update the global npm package through the package manager that installed it. Restart Graft after changing the runtime.
...
3. Check the custom binary path in Graft.
4. Restart Graft after installation or shell changes.

### No models are available

Run Pi directly and use `/login`, or configure a supported provider API key. Confirm `/model` shows at least one authenticated model before refreshing Graft.

If Anthropic authentication works but Fable 5.1 is missing, refresh Pi in Graft and start a new task. Graft can repair the known Anthropic catalog entry, but it cannot grant model access that the configured account or provider rejects.
