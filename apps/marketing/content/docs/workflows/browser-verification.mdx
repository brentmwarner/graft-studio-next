...
description: Verify visible web behavior with Graft’s shared in-app browser, semantic snapshots, screenshots, logs, and deliberate retries.
---

Graft’s in-app browser is a shared Chromium page surface: you and the agent can inspect the same page. Tabs and browser tool access belong to the task, while cookies and signed-in sessions are shared across Graft browser tabs and agent workflows. Use it to verify visible behavior alongside code review and tests.
...
These are two presentations of one task-scoped browser session, not separate browsers. Tabs, navigation, and the page an agent operates remain attached to the task when you move between them. Cookies and signed-in sessions use the shared Graft browser session. When the floating panel owns the live native page, the hidden sidebar stays preview-only so two surfaces cannot send input to the same guest.
...
Use the task’s assigned browser tab. Browser tabs are scoped to the Graft thread; do not reuse a tab identifier from another task.
