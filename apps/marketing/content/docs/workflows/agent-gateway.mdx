---
title: Agent Gateway
description: Let a supported Graft provider session inspect, create, coordinate, and monitor ordinary Graft tasks through app-owned MCP tools.
---

The Graft Agent Gateway is an internal, thread-scoped MCP control surface injected into supported provider sessions. It lets one Graft task coordinate other Graft resources while Graft continues to enforce project, worktree, runtime-mode, and task-lifecycle rules.
...
- Create one or more ordinary Graft tasks
...
The gateway does not create a separate orchestration database. Its tools delegate to Graft’s existing task, provider, Git, automation, diagnostics, and browser services.

## Start with context and capabilities

A coordinating agent should orient itself before writing anything:

1. Call `graft_context` to confirm the caller thread, active turn, provider, project, and granted coordination capabilities.
2. Call `graft_capabilities` to discover provider/model targets, option shapes, availability, and operation limits.
3. Use `graft_list_projects` when the work may target another project.
4. Use `graft_list_threads` and `graft_read_thread` to avoid duplicating existing work.
...
Use `graft_create_thread` when the plan contains exactly one new task.
...
Use `graft_create_threads` for an exact batch of 1–20 standalone tasks.
...
Use `graft_wait_for_threads` to wait for authorized child tasks rather than repeatedly listing state or sending status messages.
...
Use `graft_send_message` for a specific correction or follow-up. Do not flood a running task with repeated status requests.
...
- Project and thread access remain constrained by Graft’s authorization model.
...
- A Graft top-level task
- A Graft worktree
- A separately resumable task
- An independent Git owner
- A resource managed through the Agent Gateway

Use provider-native subagents for bounded work inside one owner task. Use the Agent Gateway when you need durable, independently visible Graft tasks.

## Browser tools

Supported provider sessions can receive the shared `browser_*` tools through the gateway. These control the task-scoped Graft browser runtime, not the desktop or another chat.
...
- [ ] Native subagents are not confused with Graft tasks
