...
| Agent Gateway orchestration | One Graft task coordinates other Graft tasks     | Structured multi-task execution and monitoring |
...
Create separate Graft tasks when work needs its own:
...
Some providers can create child agents or tasks inside one provider session. Graft can project supported child activity into the task UI, including identity, status, and tool activity.
...
The parent task remains the owner. Provider-native children are not automatically separate Graft tasks, branches, or worktrees.
...
The [Agent Gateway](/docs/workflows/agent-gateway) lets supported Graft provider sessions create, read, wait for, steer, interrupt, title, and archive ordinary Graft tasks through app-owned MCP tools.
