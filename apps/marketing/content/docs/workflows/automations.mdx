---
title: Automations
description: Schedule recurring Graft work with reviewable runs, bounded ownership, and explicit stop conditions.
---

Automations schedule agent work through the same task, provider, environment, and turn pipeline as interactive Graft work. They remove the need to start every run manually; they do not remove the need to constrain, monitor, and review the result.
...
- How many consecutive failures are allowed before Graft disables the automation
...
Using targets discovered from `graft_capabilities`, agent-authored standalone and dedicated automations can specify an exact provider, model, and supported options. Graft validates that target against live provider availability and model discovery in the automation project before saving it. When the user explicitly requests creation, the agent can leave the automation disabled (`enabled: false`) for review; inspect its saved model selection before enabling runs.
...
A heartbeat automation repeats until its natural-language stop condition is satisfied. After a successful run, Graft can evaluate the condition and disable the automation with a recorded reason.
...
The **On failure** policy controls how many consecutive failed runs Graft tolerates before disabling the automation:
...
Check whether the automation is paused, the schedule and timezone are correct, Graft is available, and the provider is installed and authenticated.
