...
The Graft browser is a shared Chromium surface scoped to the owning task. Browser failures are safest to handle with an observe–act–observe loop: inspect the current page, perform one bounded action, then inspect the resulting state.

Tabs and browser tool access belong to the task. Cookies and signed-in sessions are shared across Graft browser tabs and agent workflows; see [Browser logins & sessions](/docs/features/browser-sessions).
...
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Safari access denied on macOS                     | Enable the correct Graft app in Full Disk Access, quit and reopen it, then retry. Reopen **Settings → General → Safari import setup** to locate the running app. |
...
| Native reader unavailable                         | Reinstall Graft with its optional native dependencies, or sign in directly.                                                                                      |
