...
description: Diagnose waiting, stuck, interrupted, failed, or inconsistent Graft tasks without losing the working state.
---
...
Pending Claude questions and draft answers can be restored after navigation or restart. If the provider session has expired, Graft shows an expired-question notice instead of submitting answers to a dead request. Choose **Restore answers** to put the saved answers into the composer, review them, then send them as a new message. **Dismiss** removes that recovery draft. Restoring alone does not send anything.
...
4. Start a new provider session in the same task when Graft supports that recovery path.
...
A new provider session may preserve the Graft task and working environment without preserving every provider-native internal state.
...
This separates a repository-specific failure from a provider-wide or Graft-wide failure.

## Journal or event-delivery warnings

Graft persists orchestration state and provider runtime events. A warning about a journal, delivery, or quarantined provider event should be treated as an operational failure, not dismissed as cosmetic.
...
4. Use `graft_diagnose_thread` when an authorized Agent Gateway session is available.
