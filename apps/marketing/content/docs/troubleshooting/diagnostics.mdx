...
| ---------------------------------- | ------------------------------------------------ | -------------------------------------------------------- |
| `graft_diagnose_thread`            | Bounded forensic snapshot across several sources | First-pass triage of a stuck or inconsistent task        |
| `graft_read_thread_activity`       | Projected task activity                          | What Graft presented as user-facing task activity        |
| `graft_read_thread_events`         | Durable orchestration event journal              | Authoritative ordering of persisted orchestration events |
| `graft_read_thread_runtime_events` | Retained provider-runtime events                 | What the provider adapter emitted before projection      |
...
```text
graft_diagnose_thread({ threadId })
```
...
Use `graft_read_thread_activity` when the question is:
...
Use `graft_read_thread_events` when ordering and durable persistence matter.
...
Use `graft_read_thread_runtime_events` when the question is:
...
1. Graft version and build type
2. Operating system
...
8. A redacted `graft_diagnose_thread` result
