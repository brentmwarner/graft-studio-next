...
description: Create a minimal, reproducible Graft bug report with useful evidence and no exposed secrets.
---
...
1. Search existing [Graft issues](https://github.com/brentmwarner/graft-studio-next/issues).
...
```text
Starting state:
- Graft packaged release 0.x.y on macOS
...
- Client type
- Whether `graft_overview` succeeds
...
Start with a redacted `graft_diagnose_thread` result when available. Add one narrower source only when it answers a specific question.
...
- Do not upload the complete Graft data directory
...
Open a new report in [Graft issues](https://github.com/brentmwarner/graft-studio-next/issues/new) only after this checklist is complete.
