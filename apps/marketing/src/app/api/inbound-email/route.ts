...
const FEEDBACK_ADDRESS = "feedback@graftapp.io";
const FORWARD_FROM = `Graft Feedback <${FEEDBACK_ADDRESS}>`;
...
export async function POST(request: Request): Promise<Response> {
  ...
  const forwardTo = process.env.GRAFT_FEEDBACK_FORWARD_TO_EMAIL?.trim();
  ...
  const { error } = await resend.emails.receiving.forward(
    ...
    {
      idempotencyKey: `graft-feedback-forward/${event.data.email_id}`,
    },
  );
  if (error) {
