...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.graft/worktrees/graft-mvp/graft-4e609bb8",
    );
    expect(result).toBe("graft-4e609bb8");
  });

  it("keeps legacy .graft worktree paths readable after migration", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.graft/worktrees/graft-mvp/graft-legacy123",
    );
    expect(result).toBe("graft-legacy123");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.graft\\worktrees\\graft-mvp\\graft-4e609bb8",
    );
    expect(result).toBe("graft-4e609bb8");
  });

  it("uses the final segment even when outside ~/.graft/worktrees", () => {
