...
import type { DeviceUdid } from "@graft/contracts";
...
export function useDeviceVideoStream(input: {
  ...
  useEffect(() => {
    ...
    const configureDecoder = (frame: DeviceFrame) => {
      ...
      if (!codec) {
        failStream("The simulator stream sent parameters Graft could not read.");
        return;
      }
      teardownDecoder();
      ...
      setStatus({
        ...
        message:
          reason === "decode-failed"
            ? "The simulator stream sent a frame Graft could not read."
