...
export function buildExternalMcpClientConfiguration(
  ...
  if (client === "codex") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Codex will save Graft as a local MCP server; then open a new Codex task."
        : "Run this command in Terminal. Codex will save Graft as a local MCP server; then open a new Codex task.",
    };
  }
  ...
  if (client === "claudeCode") {
    ...
    return {
      ...
      instruction: /win/i.test(platform)
        ? "Run this command in PowerShell. Claude Code will make Graft available in all your projects."
        : "Run this command in Terminal. Claude Code will make Graft available in all your projects.",
    };
  }
  ...
  return {
    ...
    instruction:
      client === "claudeDesktop"
        ? "In Claude Desktop, open Settings → Developer → Edit Config. Add the Graft entry without removing existing servers, save, and restart Claude Desktop."
...
export function buildExternalMcpSetupPrompt(input: {
  ...
  const sections: string[] = [
    "Connect this coding agent to Graft via MCP. Complete every step yourself, in order, and report what happened.",
  ];
