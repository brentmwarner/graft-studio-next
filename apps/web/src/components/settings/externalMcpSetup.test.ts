...
describe("external MCP guided setup", () => {
  it("builds copyable Codex and Claude Code commands without embedding a credential", () => {
    ...
    expect(codex.value).toBe(
      "codex mcp add graft --env ELECTRON_RUN_AS_NODE=1 -- /Applications/Graft.app/Contents/MacOS/Graft server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Graft home'",
    );
    expect(claude.value).toBe(
      "claude mcp add --scope user graft -e ELECTRON_RUN_AS_NODE=1 -- /Applications/Graft.app/Contents/MacOS/Graft server.js mcp serve --integration mcp_int_example --home-dir '/tmp/Graft home'",
    );
  ...
  it("builds standard JSON configuration for desktop and other clients", () => {
    ...
    const parsed = JSON.parse(desktop.value) as {
      mcpServers: { graft: { command: string; args: ReadonlyArray<string> } };
    };

    expect(desktop.format).toBe("json");
    expect(parsed.mcpServers.graft).toEqual(stdio);
  });
  ...
  it("builds a project-specific prompt without exposing implementation identifiers", () => {
    const prompt = buildExternalMcpExamplePrompt("Graft app");

    expect(prompt).toContain('project named "Graft app"');
  ...
  it("builds one agent-facing setup prompt covering pairing, registration, and verification", () => {
    const prompt = buildExternalMcpSetupPrompt({
      setupCommand: "graft mcp pair --code syn_pair_v1_example --home-dir /tmp/home",
      stdio,
    });

    expect(prompt).toContain("syn_pair_v1_example");
    expect(prompt).toContain("codex mcp add graft");
    expect(prompt).toContain("claude mcp add --scope user graft");
    expect(prompt).toContain('"mcpServers"');
    expect(prompt).toContain("graft_overview");
