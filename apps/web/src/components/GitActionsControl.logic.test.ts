...
describe("resolveDefaultCreateBranchName", () => {
  it("uses Graft as the default namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "fix toast copy");
    assert.equal(branch, "graft/fix-toast-copy");
  });

  it("normalizes an existing legacy graft namespace", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "graft/refine-toolbar-actions");
    assert.equal(branch, "graft/refine-toolbar-actions");
  });

  it("preserves nested namespaces under Graft", () => {
    const branch = resolveDefaultCreateBranchName(["main"], "feature/refine-toolbar-actions");
    assert.equal(branch, "graft/feature/refine-toolbar-actions");
  });
...
describe("resolveLiveThreadBranchUpdate", () => {
  it("does not regress a semantic thread branch back to a temporary worktree branch", () => {
    const update = resolveLiveThreadBranchUpdate({
      ...
      gitStatus: status({ branch: "graft/deadbeef" }),
    });
...
describe("shouldOfferCreateBranchPrompt", () => {
  const temporaryBranch = "graft/deadbeef";
