...
import { type MessageId, type TurnId } from "@graft/contracts";
...
function workLogGraftThreadCreationsEqual(
  a: WorkLogEntry["graftThreadCreation"],
  b: WorkLogEntry["graftThreadCreation"],
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (
...
function workLogEntryContentEqual(a: WorkLogEntry, b: WorkLogEntry): boolean {
  return (
    ...
    workLogGraftThreadCreationsEqual(a.graftThreadCreation, b.graftThreadCreation) &&
