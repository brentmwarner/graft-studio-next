...
describe("electronUpdaterSecurity", () => {
  ...
  it("parses distinguished names the same way as builder-util-runtime", () => {
    const parsed = parseDistinguishedName('CN=Graft, O="Acme, Inc.", OU=Tools\\2C Desktop');

    expect(parsed.get("CN")).toBe("Graft");
  ...
  it("uses only embedded full publisher DNs and never feed-controlled names", () => {
    expect(
      resolveWindowsUpdatePublisherNames(
        ...
        [" CN=Graft, O=Acme Tools ", "CN=Only", ""],
      ),
    ).toEqual(["CN=Graft, O=Acme Tools"]);
  ...
  it("validates a matching full distinguished name through the injected execFile seam", async () => {
    const execFile = vi.fn((file, args, options, callback) => {
      callback(
        ...
        JSON.stringify({
          Status: 0,
          Path: "C:\\Users\\test\\AppData\\Local\\Temp\\GraftSetup.exe",
          SignerCertificate: {
            Subject: "CN=Graft, O=Acme Tools",
          },
        }),
        "",
      );
    ...
    const result = await verifyWindowsUpdateCodeSignature(
      ["CN=Graft, O=Acme Tools"],
      "C:\\Users\\test\\AppData\\Local\\Temp\\GraftSetup.exe",
  ...
  it("rejects a CN-only publisher allowlist", async () => {
      ...
      logger,
      {
        ...
        execFile: vi.fn((_file, _args, _options, callback) => {
          callback(
            ...
            JSON.stringify({
              Status: 0,
              Path: "C:\\Temp\\GraftSetup.exe",
              SignerCertificate: { Subject: "CN=Graft, O=Acme Tools" },
            }),
            "",
          );
        }),
    ...
    expect(result).toContain("publisherNames: CN=Graft");
  ...
  it("returns a mismatch summary for an unexpected publisher", async () => {
    ...
    expect(result).toContain("publisherNames: CN=Graft, O=Acme Tools");
  ...
  it("patches electron-updater BaseUpdater spawnSyncLog only on Windows", () => {
    ...
    expect(prototype.__graftSpawnSyncLogPatched).toBe(true);
  });
