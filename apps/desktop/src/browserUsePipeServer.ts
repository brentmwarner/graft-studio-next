...
import type { BrowserToolName, ThreadId } from "@graft/contracts";
...
const PIPE_DIR = "graft-browser-host";
const PIPE_NAME_PREFIX = "graft-browser-host";

export const GRAFT_BROWSER_HOST_PIPE_ENV = "GRAFT_BROWSER_HOST_PIPE_PATH";
export const GRAFT_BROWSER_HOST_CAPABILITY_ENV = "GRAFT_BROWSER_HOST_CAPABILITY";
export const GRAFT_BROWSER_HOST_CAPABILITY_FD_ENV = "GRAFT_BROWSER_HOST_CAPABILITY_FD";
/** @deprecated Read/written only while old backend builds are still supported. */
export const GRAFT_BROWSER_USE_PIPE_ENV = "GRAFT_BROWSER_USE_PIPE_PATH";
...
export function resolveConfiguredBrowserHostPipePath(
  ...
  const configured =
    env[GRAFT_BROWSER_HOST_PIPE_ENV]?.trim() || env[GRAFT_BROWSER_USE_PIPE_ENV]?.trim();
  return configured || resolveDefaultBrowserHostPipePath(platform);
}

export const GRAFT_BROWSER_HOST_PIPE_PATH = resolveConfiguredBrowserHostPipePath();
...
export function resolveBrowserHostPipeBackendEnv(
  ...
  delete backendEnv[GRAFT_BROWSER_HOST_PIPE_ENV];
  delete backendEnv[GRAFT_BROWSER_USE_PIPE_ENV];
  delete backendEnv[GRAFT_BROWSER_HOST_CAPABILITY_ENV];
  delete backendEnv[GRAFT_BROWSER_HOST_CAPABILITY_FD_ENV];
  const pipePath = activePipePath?.trim();
  if (pipePath && Number.isInteger(capabilityFd) && (capabilityFd ?? 0) >= 3) {
    backendEnv[GRAFT_BROWSER_HOST_PIPE_ENV] = pipePath;
    backendEnv[GRAFT_BROWSER_USE_PIPE_ENV] = pipePath;
    backendEnv[GRAFT_BROWSER_HOST_CAPABILITY_FD_ENV] = String(capabilityFd);
  }
  return backendEnv;
}
...
export class BrowserHostPipeServer {
  ...
  constructor(
    ...
    options: BrowserHostPipeServerOptions | string = GRAFT_BROWSER_HOST_PIPE_PATH,
  ) {
    const normalized = typeof options === "string" ? { pipePath: options } : options;
    this.platform = normalized.platform ?? process.platform;
    this.pipePath = normalized.pipePath ?? GRAFT_BROWSER_HOST_PIPE_PATH;
