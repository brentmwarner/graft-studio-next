...
function makeIsolatedServerEnvironment(input: {
  ...
  return {
    ...environment,
    GRAFT_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "0",
    GRAFT_CLAUDE_KEEPALIVE: "0",
    GRAFT_DESKTOP_SHUTDOWN_TOKEN: input.shutdownToken,
    GRAFT_HOME: input.homeDir,
    GRAFT_BIND_HOST: "127.0.0.1",
    GRAFT_MODE: "desktop",
    GRAFT_NO_BROWSER: "1",
    GRAFT_PORT: String(input.port),
  };
}
...
describeWindows("Windows desktop backend shutdown integration", () => {
  ...
  it("authenticates a real server, deduplicates shutdown, drains its runtime, and clears its finalizer artifact", async () => {
    ...
    try {
      const homeDir = await resources.makeTempDir("graft-windows-shutdown-");
