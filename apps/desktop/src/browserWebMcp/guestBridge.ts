...
/**
 * Install the page-facing WebMCP compatibility API and Graft's private bridge
...
export function installWebMcpBridgeInMainWorld(hostAllowsCompatibility = false): void {
  ...
  const BRIDGE_PROPERTY = "__graftWebMcpBridgeV1";
  ...
  const safeError = (error: unknown): { readonly name: string; readonly message: string } => {
    ...
    try {
      ...
      // let that hostile error value escape Graft's bounded error envelope.
    }
    return {
  ...
  const declarativeForm = Symbol("graft.webmcp.declarative-form");
  ...
  const normalizeTool = async (
    ...
    const descriptor = {
      ...
      annotations: {
        readOnlyHint: tool.annotations?.readOnlyHint === true,
        // All page-provided metadata and results are untrusted to Graft even
  ...
  const bridge = Object.freeze({
    ...
    async invoke(index: number, signature: string, inputJson: string, invocationId: string) {
      ...
      try {
        ...
        // so Graft remains compatible with early-preview Chromium builds.
