...
function makeTempDir(): string {
  const directory = FS.mkdtempSync(Path.join(OS.tmpdir(), "graft-profile-test-"));
...
describe("desktopUserDataProfile", () => {
  it("resolves the canonical Graft profile names", () => {
    const appDataBase = "/Users/tester/Library/Application Support";
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "graft-dev" })).toBe(
      "/Users/tester/Library/Application Support/graft-dev",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "graft" })).toBe(
      "/Users/tester/Library/Application Support/graft",
    );
    expect(resolveDesktopUserDataPath({ appDataBase, userDataDirectoryName: "graft-canary" })).toBe(
      "/Users/tester/Library/Application Support/graft-canary",
    );
  });
  ...
  it("uses an explicit smoke profile instead of the development profile", () => {
    expect(
      resolveDesktopUserDataPath({
        ...
        userDataDirectoryName: "graft-dev",
        testOverridePath: "/tmp/graft-desktop-smoke/electron-user-data",
      }),
    ).toBe("/tmp/graft-desktop-smoke/electron-user-data");
  });
  ...
  it("repairs missing browser data from the profile recorded by the bridge", () => {
    ...
    const targetPath = Path.join(appDataBase, "graft");
    ...
    const targetPartitionPath = Path.join(targetPath, "Partitions", "graft-browser");
    ...
    FS.writeFileSync(
      Path.join(targetPath, "graft-profile-seed.json"),
  ...
  it("rejects bridge manifests that point outside the Graft profile parent", () => {
    const appDataBase = makeTempDir();
  ...
  it("never adds a foreign SQLite sidecar beside an existing Graft database", () => {
    const appDataBase = makeTempDir();
    const targetPath = Path.join(appDataBase, "graft");
    const sourcePath = Path.join(appDataBase, "previous-profile");
    const sourcePartitionPath = Path.join(sourcePath, "Partitions", "previous-browser");
  ...
  it("replaces an orphaned target sidecar with one from the repaired database generation", () => {
    ...
    expect(
      FS.readdirSync(targetPartitionPath).some((entryName) =>
        entryName.startsWith(".graft-bridge-"),
      ),
    ).toBe(false);
  });
  ...
  it("ignores a malformed bridge manifest without attempting a repair", () => {
    ...
    const targetPath = Path.join(appDataBase, "graft");
    FS.mkdirSync(targetPath, { recursive: true });
    FS.writeFileSync(Path.join(targetPath, "graft-profile-seed.json"), "{");
