...
const credential = {
  ...
  uplinkSecret: "test-uplink-secret-12345678",
  httpBaseUrl: "https://relay.test/e/env-test",
  wsBaseUrl: "wss://relay.test/e/env-test",
...
function setup(signedIn = true) {
  const secrets = new Map<string, string>(
    signedIn ? [["graft-account-token", "account-token"]] : [],
  );
  ...
  const controller = new MobileRelayController({
    controlPlaneBaseUrl: "https://relay.test",
    label: "Graft Dev",
    requestBackend,
    fetchImpl,
    uplinkFactory,
...
describe("Graft desktop relay", () => {
  it("requires account sign-in and never registers anonymously", async () => {
    ...
    expect(requestBackend).toHaveBeenLastCalledWith(
      "/api/graft/connections/relay",
  ...
  it("registers once and renews a relay URL without sending secrets to the backend", async () => {
    ...
    expect(requestBackend).toHaveBeenLastCalledWith(
      "/api/graft/connections/relay",
      expect.objectContaining({ state: "connected", httpBaseUrl: credential.httpBaseUrl }),
    );
