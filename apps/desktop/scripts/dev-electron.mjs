...
function cleanupStaleDevApps() {
  ...
  const commandPattern = `^${executable}[[:space:]]+--graft-dev-root=${devRoot}([[:space:]]|$)`;
  spawnSync("pkill", ["-f", "--", commandPattern], { stdio: "ignore" });
}

function listStaleComputerUsePids() {
  // Only macOS exposes a verifiable Graft (Dev) executable path for these
  ...
  return candidatePids.filter((pid) => {
    ...
    if (!/(?:Graft|Graft) \(Dev\)\.app\/Contents\/MacOS\/Electron/.test(command)) {
      return false;
    }
...
function cleanupStaleComputerUseApps() {
  ...
  console.error(
    `[desktop-dev] Cleaning up ${stalePids.length} stale Graft (Dev) Computer Use helper process${stalePids.length === 1 ? "" : "es"} from other worktrees.`,
  );
...
function warnIfAlphaAppRunning() {
  ...
  const pids = listPidsByExactProcessName("Graft").filter((pid) =>
    readProcessCommand(pid).startsWith("/Applications/Graft.app/Contents/MacOS/Graft"),
  );
  if (pids.length === 0) {
    return;
  }
  ...
  console.error(
    "[desktop-dev] Graft is still running. Close it before testing voice in Graft (Dev), or you may be looking at the wrong app/runtime.",
  );
  console.error(`[desktop-dev] Running Graft process IDs: ${pids.join(", ")}`);
}
...
function startApp() {
  ...
  const app = spawn(
    resolveElectronPath(),
    [`--graft-dev-root=${desktopDir}`, "dist-electron/main.js"],
    {
      cwd: desktopDir,
      env: {
        ...childEnv,
