# Graft Code-Quality Branch — Complete Review Handoff
...
- Effect remains in Graft for lifecycle, cancellation, queues, errors, and application policy.
...
| Commit              | Summary                                      |                                         Size |
| ------------------- | -------------------------------------------- | -------------------------------------------: |
| `0e5f6af9c`         | Refactor Graft orchestration and web UI flow |                  387 files, +43,323 / -9,299 |
...
AcpSessionRuntime
  ...
  └─ Graft: lifecycle, bounds, logging, policy, normalized events
        │
        ▼
...
| Workstream                              | Why it was needed                                                                   | Main change                                                             | Result to verify                                                       |
| --------------------------------------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `P2-ACP-01` ACP foundation              | Graft maintained a custom wire stack beside the official SDK                        | Official SDK owns all production ACP wire behavior; custom wire deleted | Grok, Droid, and Cursor have one wire implementation and no fallback   |
...
Use the official TypeScript SDK for the protocol wire, while keeping Effect for Graft's application
lifecycle.
...
| Provider process supervision                  | Graft/Effect           |
| Queue and resource limits                     | Graft/Effect           |
| Session/product policy                        | Graft/Effect           |
| Normalized Graft events                       | Graft/Effect           |
| Error translation into Graft domain errors    | Thin local adapter     |
...
3. Provider adapters should depend on a stable Graft runtime interface, not choose a protocol
   implementation.
...
The package currently retains only generated/schema/error adapters still imported by Graft. A
...
SQLite persistence, or the full Graft app.
...
```text
Review the Graft branch `codex/audit-code-quality` as a senior architecture, correctness,
