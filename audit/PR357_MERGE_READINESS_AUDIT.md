# Graft PR #357 — Final code-quality, ACP, and provider audit
...
- after all tests, direct children of the Graft server totalled about 1.84 GiB RSS, dominated by
  OpenCode;
...
as the sole protocol/wire authority and keep Effect above it as Graft's lifecycle and application
...
- `AcpSessionRuntime.ts` constructs `OfficialAcp.client({ name: "graft" })`, registers client
...
client/agent builders, handlers, streams, validation, and connection lifecycle that Graft would
...
| Protocol upgrades              | Graft had to regenerate and reconcile behavior | Follow the official SDK and negotiate protocol capabilities         |
| Process lifecycle              | Effect                                         | Effect, unchanged                                                   |
| Backpressure and queue budgets | Effect/custom transport                        | Graft-owned bounded bridge around the official SDK                  |
| Canonical provider events      | Graft adapters                                 | Graft adapters, unchanged                                           |
...
- Closing the browser reduced the retained process from roughly 651 MiB to 135 MiB; stopping Graft
...
- [ ] Do not recreate official SDK types in a new Graft package.
