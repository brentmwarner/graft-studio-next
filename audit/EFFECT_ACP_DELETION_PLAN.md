...
process supervision, teardown, and Graft lifecycle policy. The official
...
2. `effect` for Graft runtime and lifecycle ownership.
3. Two small local modules beside the ACP runtime:
   - `AcpErrors.ts`, containing only the Effect-native errors Graft still consumes;
...
- `AcpErrors.ts` owns only Graft's three Effect-native runtime errors, and `AcpExtensions.ts` owns
...
1. A minimal Effect codec for the parts of `SessionConfigOption` Graft reads.
...
3. Delete the redundant Effect-Schema decode of `session/update`; the official SDK already validates
   it before invoking Graft's handler.
...
7. Normalize SDK `null` values at consumption sites where Graft uses `undefined`; do not introduce
...
- Effect still owns Graft runtime/lifecycle policy;
